...
resetPrimed mid cid (PrimedWork pw) =
  PrimedWork $!
    ...
    Just (T2 (payload, primedParentHash) extension)
      ...
      | otherwise -> do
          ...
          logFun @T.Text Info $
            "newWork: chain "
              <> sshow cid
              <> " not mineable because of parent header mismatch"
              <> ". Primed parent hash: "
              <> toText primedParentHash
              <> ". Extension parent: "
              <> toText (_blockHash extensionParent)
              <> ". Extension height: "
              <> sshow (_blockHeight extensionParent)
...
publish lf cdb pwVar miner pd s = do
  ...
  try (extend c pd s) >>= \case
    ...
    Right (bh, Just ch) -> do
      ...
      let bytes =
            sum . fmap (BS.length . _transactionBytes) $
