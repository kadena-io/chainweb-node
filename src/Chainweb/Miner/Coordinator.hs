...
updateForCut
    ...
    -> Cut
    -> IO ()
updateForCut hdb caches ms c = do
    ...
    forChain t cid var cache =  do
        ...
        case onParents t ps cur of
            ...
            Just !new
                ...
                | otherwise -> getLatestIO cache (_workRankedHash cur) >>= \case
                    Nothing -> atomically $ writeTVar var new
                    Just pld -> case onPayload t pld new of
                        Nothing -> return ()
                        Just !newnew -> atomically $ writeTVar var newnew

updateForPayload :: MiningState -> NewPayload -> IO ()
updateForPayload ms pld = do
    t <- BlockCreationTime <$> getCurrentTimeIntegral
    cur <- readTVarIO var
    case onPayload t pld cur of
        Nothing -> return ()
        Just !new -> atomically $ writeTVar var new
  where
    cid = _chainId pld
    var = ms ^?! ixg cid

updateForSolved :: MiningState -> SolvedWork -> IO ()
updateForSolved ms sw = do
    cur <- readTVarIO var
...
runCoordination
    ...
    -> IO ()
runCoordination mr =
    runForever lf "miningCoordination" $
        eventStream cdb caches
            & S.mapM_
                ...
                -- STM variable, too.
                (either (updateForCut f caches state) (updateForPayload state))
  where
    lf :: LogFunction
...
-- item and any previous items that have not been consumed.
--
...
solve mr solved@(SolvedWork hdr) =
    lookupIO cache cacheKey (view blockPayloadHash hdr) >>= \case
        ...
        Nothing -> throwM NoAsscociatedPayload
            -- FIXME Why not log this directly here?
        ...
        Just np -> do
            ...
            try (extend c pld pwo solved) >>= \case
                ...
                Right (bh, Just ch) -> do
                    updateForSolved (_coordState mr) solved
                    publish cdb ch
  ...
  where
    cdb = _coordCutDb mr
    cache = (_coordPayloadCache mr) HM.! _chainId solved
    cacheKey = _rankedBlockHash hdr
