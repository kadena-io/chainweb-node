...
resetPrimed mid cid (PrimedWork pw) = PrimedWork
    $! HM.adjust (HM.adjust (\(T2 ph _) -> T2 ph Nothing) cid) mid pw
...
newWork logFun choice eminer@(Miner mid _) hdb pact tpw c = do
    ...
    mpw :: HM.HashMap ChainId (T2 ParentHeader (Maybe PayloadData)) <- atomically $ do
        PrimedWork pw <- readTVar tpw
        mpw <- maybe retry return (HM.lookup mid pw)
        guard (any (isJust . ssnd) mpw)
        return mpw
    let mr :: Maybe (T2 (T2 ParentHeader (Maybe PayloadData)) CutExtension) = T2
            <$> HM.lookup cid mpw
