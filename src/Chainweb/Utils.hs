...
alignWithV f a b =
  V.zipWith (\a' -> f . These a') a b <> case (V.length a, V.length b) of
    (la, lb)
      | la == lb -> mempty
...
parseM p =
  either (throwM . TextFormatException . T.pack) return
...
unexpectedMsg msg expected actual =
  msg
    <> ", expected: "
    <> sshow (getExpected expected)
    <> ", actual: "
    <> sshow (getActual actual)
...
catchSynchronous ::
  ...
  (e -> m a) ->
  m a
catchSynchronous a f =
  force
    <$> a
    `catches` [ Handler $ throwM @_ @SomeAsyncException,
                Handler f
              ]
...
instance FromJSON (a -> a) => FromJSON (EnableConfig a -> EnableConfig a) where
  parseJSON = withObject "EnableConfig" $ \o ->
    id
...
pEnableConfig compName pConfig =
  id
    <$< enableConfigEnabled
      .:: enableDisableFlag
        % long compName
...
timeoutStream msecs = go
  where
    go s =
      lift (timeout msecs (S.next s)) >>= \case
...
approximateThreadDelay d =
  withMVar threadDelayRng (approximately d)
    >>= threadDelay
...
unsafeManagerWithSettings settings =
  HTTP.newTlsManagerWith $
    settings $
      HTTP.mkManagerSettings (HTTP.TLSSettingsSimple True True True) Nothing

setManagerRequestTimeout :: Int -> HTTP.ManagerSettings -> HTTP.ManagerSettings
setManagerRequestTimeout micros settings =
  settings
