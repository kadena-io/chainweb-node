...
effectiveWindow h =
  WindowWidth <$> case window ver of
    Nothing -> Nothing
    Just (WindowWidth w)
...
powTarget p@(ParentHeader ph) as bct = case effectiveWindow ph of
  ...
  where
    t =
      EpochStartTime $
        if oldTargetGuard ver (_blockHeight ph)
          then _bct bct
...
getAdjacentHash p b =
  firstOf (blockAdjacentHashes . ixg (_chainId p)) b
...
_blockPow h =
  powHash (_blockChainwebVersion h) $
    runPutS $
      encodeBlockHeaderWithoutHash h
