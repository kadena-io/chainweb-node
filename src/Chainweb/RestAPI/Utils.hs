...
-- 'HasClient' instance.
type ChainwebEndpointApiWithHeader c a =
  "chainweb"
    :> Version
    :> ChainwebVersionSymbol c
...
instance
  ...
  where
  type
    ServerT ('ChainwebEndpoint c :> api) m =
...
instance
  ...
  where
  type
    Client m ('ChainwebEndpoint v :> api) =
...
instance
  ...
  where
  type
    MkLink ('ChainwebEndpoint v :> api) a =
...
instance
  ...
  where
  type
    ServerT ('NetworkEndpoint ('ChainNetworkT c) :> api) m =
...
instance
  ...
  where
  type
    ServerT ('NetworkEndpoint ('MempoolNetworkT c) :> api) m =
...
instance HasServer api ctx => HasServer ('NetworkEndpoint 'CutNetworkT :> api) ctx where
  type
    ServerT ('NetworkEndpoint 'CutNetworkT :> api) m =
...
instance
  ...
  where
  type
    Client m ('NetworkEndpoint ('ChainNetworkT c) :> api) =
...
instance
  ...
  where
  type
    Client m ('NetworkEndpoint ('MempoolNetworkT c) :> api) =
...
instance (HasClient m api) => HasClient m ('NetworkEndpoint 'CutNetworkT :> api) where
  type
    Client m ('NetworkEndpoint 'CutNetworkT :> api) =
...
instance
  ...
  where
  type
    MkLink ('NetworkEndpoint ('ChainNetworkT c) :> api) a =
...
instance
  ...
  where
  type
    MkLink ('NetworkEndpoint ('MempoolNetworkT c) :> api) a =
...
instance (HasLink api) => HasLink ('NetworkEndpoint 'CutNetworkT :> api) where
  type
    MkLink ('NetworkEndpoint 'CutNetworkT :> api) a =
...
data SomeApi
  = forall (a :: Type).
    (HasServer a '[], HasClient ClientM a) =>
    SomeApi (Proxy a)
...
type family RespBodyContentTypeNotSupportedMsg ct api where
  RespBodyContentTypeNotSupportedMsg ct api =
    TypeError
...
type family ReqBodyContentTypeNotSupportedMsg ct api where
  ReqBodyContentTypeNotSupportedMsg ct api =
    TypeError
