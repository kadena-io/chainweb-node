...
data PactExecutionService = PactExecutionService
    ...
    , _pactLocal :: !(
        Bool ->
        ChainwebTransaction ->
        IO (Either PactException (Either MetadataError (CommandResult Hash))))
...
mkWebPactExecutionService hm = WebPactExecutionService $ PactExecutionService
    ...
    , _pactLocal = \_pf _ct -> throwM $ userError "Chainweb.WebPactExecutionService.mkPactExecutionService: No web-level local execution supported"
...
emptyPactExecutionService = PactExecutionService
    ...
    , _pactLocal = \_ _ -> throwM (userError "emptyPactExecutionService: attempted `local` call")
