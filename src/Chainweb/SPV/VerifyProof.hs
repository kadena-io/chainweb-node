...
forkedError v cid h = throwM $ SpvExceptionVerificationFailed $ if CW.chainweb227Pact v cid h then errMsgPost227 else errMsgPre227
    where
        errMsgPre227 :: Text
        errMsgPre227 = "target header is not in the chain"

        errMsgPost227 :: Text
        errMsgPost227 = "target header is not in the chain or is out of bounds"
