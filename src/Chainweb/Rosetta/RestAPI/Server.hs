    ...
    work = do
      ...
      bh <-
        findBlockHeaderInCurrFork
          cutDb
          cid
          (get _partialBlockId_index pbid)
          (get _partialBlockId_hash pbid)
      ...
      pure $!
        ConstructionDeriveResp
          { _constructionDeriveResp_address = Nothing,
            ...
            _constructionDeriveResp_metadata =
              Just $!
                toObject $!
                  DeriveRespMetaData
    ...
    work = do
      ...
      pure $!
        ConstructionPreprocessResp
          { _constructionPreprocessResp_options =
              Just $!
                toObject $!
...
constructionMetadataH v cutDb pacts (ConstructionMetadataReq net opts someKeys) =
  ...
  where
    ...
    work = do
      ...
      availableSigners <-
        someKeys
          ?? rosettaError' RosettaMissingPublicKeys
          >>= hoistEither
      ...
      pure $!
        ConstructionMetadataResp
          { _constructionMetadataResp_metadata =
              toObject $!
                PayloadsMetaData
...
constructionPayloadsH v req =
  ...
  where
    ...
    work = do
      ...
      meta :: PayloadsMetaData <-
        hoistEither $
          note
            (rosettaError' RosettaMissingMetaData)
            someMeta
            >>= extractMetaData
...
constructionParseH v (ConstructionParseReq net isSigned tx) =
  ...
  where
    ...
    work = do
      ...
      (EnrichedCommand cmd txInfo signAccts) <-
        note
...
constructionCombineH (ConstructionCombineReq _ unsignedTx sigs) =
  ...
  where
    ...
    work = do
      (EnrichedCommand unsignedCmd meta signAccts) <-
        note
          (rosettaError' RosettaUnparsableTx)
...
constructionHashH (ConstructionHashReq _ signedTx) =
  ...
  where
    ...
    work = do
      (EnrichedCommand cmd _ _) <-
        note RosettaUnparsableTx $
...
constructionSubmitH v ms (ConstructionSubmitReq net tx) =
  ...
  where
    ...
    checkResult (Left (T2 hsh insErr)) =
      throwE $
        stringRosettaError RosettaInvalidTx $
...
mempoolH ::
  ...
  NetworkReq ->
  Handler MempoolResp
mempoolH v ms (NetworkReq net _) =
  work >>= \case
