...
import Chainweb.Utils (T2 (..), int, sshow)
...
data ConstructionTx = ConstructTransfer
...
getSuggestedFee tx someMaxFees someMult = do
  ...
  where
    ...
    checkMaxFeeSufficient maxFee
      ...
      | otherwise =
          Left $
            stringRosettaError RosettaInvalidAmount $
              "max_fee: Expected a minimum fee of "
                ++ show minFeeNeeded
                ++ "KDA for specified operations, but received max_fee="
                ++ show maxFee
                ++ "KDA"
      where
        minFeeNeeded =
    ...
    estimatedGasLimit =
      P.GasLimit $ P.ParsedInteger $! case tx of
...
rosettaAccountIdtoKAccount acct = do
  ...
  ownership <-
    toRosettaError RosettaInvalidKAccount $
      note (show acct) $
...
rosettaPubKeyTokAccount (RosettaPublicKey pubKey curve) = do
  ...
  kAccount <-
    toRosettaError RosettaInvalidPublicKey $
      note (show pubKey) $
...
getScheme ct =
  Left $
    stringRosettaError RosettaInvalidPublicKey $
...
sigToScheme st =
  Left $
    stringRosettaError RosettaInvalidSignature $
...
instance ToJSON EnrichedCommand where
  toJSON (EnrichedCommand cmd tx accts) =
    object
      [ "cmd" .= cmd,
        "tx_info" .= tx,
    ...
    weave (op, newRelatedIds) =
      case newRelatedIds of
        ...
        l -> case _operation_relatedOperations op of
          Nothing ->
            op -- no previous related operations
    ...
    { _operation_operationId = OperationId idx Nothing,
      ...
      _operation_amount =
        Just $
          kdaToRosettaAmount $
            _balanceDelta $
              _accountLogBalanceDelta acctLog,
...
parseOp (Operation i _ typ stat someAcct someAmt _ someMeta) = do
  ...
  (OperationMetaData prevOwn currOwn) <-
    someMeta @?? "Missing metadata"
      >>= extractMetaData
  prevOwn @?= currOwn -- ensure that the ownership wasn't rotated
  ownership <-
    hushResult (fromJSON currOwn)
  ...
  where
    ...
    Nothing @?? msg =
      Left $
        stringRosettaError RosettaInvalidOperation $
...
parsePubKeys k v = do
  g :: (P.Guard PactValue) <-
    toRosettaError RosettaInvalidAccountProvided $
      noteResult $
        fromJSON v
  case g of
    P.GUser _ -> pure []
...
extractMetaData =
  toRosettaError RosettaUnparsableMetaData
    . noteResult
    . fromJSON
    . Object
