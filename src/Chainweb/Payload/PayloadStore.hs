...
lookupPayloadDataWithHeight' tdb height k = runMaybeT $ do
    let
      lookupOld    = tableLookupMT (_oldTransactionDbBlockPayloadsTbl tdb) k
      lookupNew h  = tableLookupMT (_newTransactionDbBlockPayloadsTbl tdb) (h, k)

    pd <- (lookupNew =<< hoistMaybe height) <|> lookupOld

    let txsHash = _blockPayloadTransactionsHash pd
    let
        lookupOld = tableLookupMT (_oldTransactionDbBlockTransactionsTbl tdb) txsHash
        lookupNew h = tableLookupMT (_newTransactionDbBlockTransactionsTbl tdb) (h, txsHash)
    txs <- (lookupNew =<< hoistMaybe height) <|> lookupOld
