...
instance (pk ~ CasKeyType (PayloadData_ a), CanReadableTransactionDbCas_ a tbl) => ReadableTable (TransactionDb_ a tbl) pk (PayloadData_ a) where
    tableLookup db k = runMaybeT $ do
        pd <- MaybeT $ tableLookup (_transactionDbBlockPayloadHeights db) k >>= \case
            Just h  -> tableLookup (_transactionDbBlockPayloads db) (h, k)
            Nothing -> tableLookup (_oldTransactionDbBlockPayloads db) k
