...
newMempoolServer = fold
    [ seg "insert" $
        endpoint methodPut "application/json" $ \mempool req resp -> do
            insertHandler mempool =<< requestFromJSON req
            resp $ responseLBS ok200 [] ""
    , seg "member" $
        endpoint methodPost "application/json" $ \mempool req resp -> do
...
insertTx :: ChainwebVersion -> ChainId -> TransactionConfig t -> [t]
    -> ApiRequest ()
insertTx v cid txcfg k0 = mkApiRequest
    methodPut
    (return . void)
    ("chainweb" /@ "0.0" /@@ v /@ "chain" /@@ cid /@ "mempool" /@ "insert")
    & requestBody .~ RequestBodyLBS (encode k)
    & requestAcceptable ?~ [maxQuality "application/json"]
    where
    k = map (T.decodeUtf8 . codecEncode (txCodec txcfg)) k0

memberTx :: ChainwebVersion -> ChainId -> [TransactionHash]
    -> ApiRequest (Either AesonException [Bool])
memberTx v cid txs = mkApiRequest
    methodPost
    (traverse jsonBody)
    ("chainweb" /@ "0.0" /@@ v /@ "chain" /@@ cid /@ "mempool" /@ "member")
    & requestBody .~ RequestBodyLBS (encode txs)
    & requestAcceptable ?~ [maxQuality "application/json"]

lookupTx
    :: ChainwebVersion -> ChainId -> TransactionConfig t -> [TransactionHash]
    -> ApiRequest (Either (Either AesonException EncodingException) [LookupResult t])
lookupTx v cid txcfg hshs = mkApiRequest
    methodPost
    (traverse $ \body -> runExceptT $ do
        j <- withExceptT Left $ ExceptT $ jsonBody body
        traverse (traverse go) j
        )
    ("chainweb" /@ "0.0" /@@ v /@ "chain" /@@ cid /@ "mempool" /@ "lookup")
    & requestBody .~ RequestBodyLBS (encode hshs)
    & requestAcceptable ?~ [maxQuality "application/json"]
    where
    go h = case decodeTx h of
        Left err -> throwError $ Right $ DecodeException $ T.pack err
        Right t -> return t
    decodeTx = codecDecode (txCodec txcfg) . T.encodeUtf8

getPendingTxs :: ChainwebVersion -> ChainId -> Maybe (ServerNonce, MempoolTxId)
    -> ApiRequest (Either AesonException PendingTransactions)
getPendingTxs v cid hw = mkApiRequest
    methodPost
    (traverse jsonBody)
    ("chainweb" /@ "0.0" /@@ v /@ "chain" /@@ cid /@ "mempool" /@ "getPending")
    & requestQuery .~ maybe []
        (\(nonce, tx) -> [("nonce", Just $ toQueryParam nonce), ("since", Just $ toQueryParam tx)])
        hw
