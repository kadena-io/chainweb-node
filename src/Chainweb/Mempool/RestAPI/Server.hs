...
insertHandler v mempool txsT = handleErrs (NoContent <$ begin)
  where
    ...
    -- | KILLSWITCH 2019-12-17T01:00:00Z: The logic involving
    -- `transferActivationDate` can be removed once the actual date has passed.
    -- Until then, this serves as an additional block against Transactions
    -- entering the system.
    --
    begin :: Handler ()
    begin = do
