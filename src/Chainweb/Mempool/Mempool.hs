...
chainwebTransactionConfig chainCtx =
  TransactionConfig
    (chainwebPayloadCodec chainCtx)
    commandHash
    chainwebTestHashMeta
    getGasPrice
    getGasLimit
  ...
  where
    ...
    commandHash c =
      let (H.Hash !h) = H.toUntypedHash $ _cmdHash c
...
instance FromJSON TransactionMetadata where
  parseJSON = withObject "TransactionMetadata" $ \o ->
    TransactionMetadata
...
chainwebTestHasher s =
  let !b = SB.toShort $ convert $ hash @_ @SHA512t_256 $ "TEST" <> s
...
instance FromJSON t => FromJSON (ValidatedTransaction t) where
  parseJSON = withObject "ValidatedTransaction" $ \o ->
    ValidatedTransaction
      <$> o .: "validatedHeight"
      <*> o .: "validatedHash"
