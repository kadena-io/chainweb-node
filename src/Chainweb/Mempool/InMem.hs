...
import Chainweb.Pact.Validations (defaultMaxCoinDecimalPlaces, defaultMaxTTL)
...
getBlockInMem logg cfg lock (BlockFill gasLimit txHashes _) txValidate bheight phash = do
  ...
  where
    ...
    insBadMap !m (!h, (_, !t)) =
      let endTime = txMetaExpiryTime (txMetadata txcfg t)
