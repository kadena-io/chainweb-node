...
import Chainweb.Time (Micros, Time, getCurrentTimeIntegral, minute, scaleTimeSpan)
import Chainweb.Utils (T2 (..), T3 (..), fromJuste, runForever, thd)
...
withMiningCoordination logger conf cdb inner
  ...
  where
    ...
    !miners =
      S.toList (_coordinationMiners coordConf)
    ...
    getPayload parent m =
      trace
        ...
        ()
        1
        (payloadWithOutputsToPayloadData <$> _pactNewBlock pact m parent)
    ...
    prune t tpw c503 c403 = runForever (logFunction logger) "MinerResources.prune" $ do
      ...
      logFunction logger Info . JsonLog $
        MiningStats
