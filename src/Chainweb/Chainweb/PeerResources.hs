...
connectionManager peerDb = do
    ...
    mgr <- HTTP.newManager settings'
        ...
        , HTTP.managerModifyResponse = \resp -> do
            ...
            newRespBody <- limitResponseBodySize p2pResponseSizeLimit (HTTP.responseBody resp)
            return
                resp {
                    HTTP.responseBody = newRespBody
                }
        }
    return (mgr, counter)
  where
...
limitResponseBodySize :: Word64 -> HTTP.BodyReader -> IO HTTP.BodyReader
limitResponseBodySize responseBodySizeLimitBytes respBody = do
    sizeCtr <- newIORef 0
    return $ loop sizeCtr
    where
    loop sizeCtr = do
        ...
        then do
            internalInvariantViolation
                $ "response body exceeded size limit of " <> sshow responseBodySizeLimitBytes <> " bytes"
        else do
            chunk <- respBody
