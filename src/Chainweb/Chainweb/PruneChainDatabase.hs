...
checkPayloads pdb False h =
  tableLookup pdb (_blockPayloadHash h) >>= \case
    Just p
...
checkPayloadsExist pdb False h =
  unlessM (tableMember db $ _blockPayloadHash h) $
    throwM $
      MissingPayloadException h
  where
...
fullGc logger rdb v = do
  ...
  where
    ...
    pruneChain cid = withBlockHeaderDb rdb v cid $ \cdb -> do
      ...
      void $
        entries cdb Nothing Nothing (Just keptBound) Nothing $
...
sweepPayloads logg db markedPayloads = do
  ...
  c0 <-
    withTableIterator payloadsTable $
  ...
  where
    go mt mo x =
      checkMark markedPayloads (_blockPayloadPayloadHash x) >>= \case
...
tryInsert cf k a =
  unlessM (member cf $ GcHash a) $
    unlessM (insert cf $ GcHash a) $
      error $
