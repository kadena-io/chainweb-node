...
import Chainweb.BlockHeader.Genesis (genesisBlockHeader, genesisBlockTarget, genesisParentBlockHash)
...
instance Show ValidationFailure where
  show (ValidationFailure p as e ts) =
    T.unpack $
      "Validation failure"
        <> ". Parent: "
        <> encodeToText (ObjectEncoded . _parentHeader <$> p)
        <> ". Adjacents: "
        <> encodeToText ((fmap (ObjectEncoded . _parentHeader)) <$> as)
        <> ". Header: "
        <> encodeToText (ObjectEncoded e)
        <> ". Description: "
        <> T.unlines (map description ts)
    where
...
validateBlockParentExists lookupParent h
  ...
  | otherwise =
      lookupParent (_blockParent h) >>= \case
...
validateAllParentsExist lookupParent h =
  runExceptT $
    WebStep
  ...
  where
    ...
    f c ph
      | genesisParentBlockHash v c == ph =
          return $
            ParentHeader $
              genesisBlockHeader v c
      | otherwise =
          lift (lookupParent $ ChainValue c ph) >>= \case
...
prop_block_genesis_target b =
  isGenesisBlockHeader b
    ==> _blockTarget b
    == genesisBlockTarget (_chainwebVersion b) (_chainId b)
