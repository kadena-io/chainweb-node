...
import Configuration.Utils hiding (Lens', (<.>), Error, disabled)
...
runChainweb cw = do
  ...
  where
    ...
    mempoolSyncClients = case _chainwebVersion cw of
        Test{} -> disabled
      ...
      where
        disabled = []
        enabled = map (runMempoolSyncClient mgr mempoolP2pConfig) chainVals
