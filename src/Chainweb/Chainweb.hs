...
withChainweb c logger rocksDb pactDbDir backupDir resetDb inner =
  ...
  where
    ...
    confWithBootstraps
      ...
      | otherwise =
          configP2p . p2pConfigKnownPeers
            %~ (\x -> L.nub $ x <> bootstrapPeerInfos v)
            $ c
...
validatingMempoolConfig cid v gl gp mv =
  Mempool.InMemConfig
  ...
  where
    ...
    preInsertSingle tx = do
      ...
      if
          | not $ assertParseChainId pcid -> Left $ Mempool.InsertErrorOther "Unparsable ChainId"
    ...
    preInsertBatch ::
      ...
      IO
        ( V.Vector
            ( Either
                (T2 Mempool.TransactionHash Mempool.InsertError)
...
withChainwebInternal conf logger peer serviceSock rocksDb pactDbDir backupDir resetDb inner = do
  ...
  where
    ...
    cutConfig =
      (defaultCutDbParams v $ _cutFetchTimeout cutConf)
    ...
    synchronizePactDb cs targetCut = do
      ...
      where
        ...
        syncOne (bh, cr) = do
          ...
          logCr Info $
            "pact db synchronizing to block "
              <> T.pack (show (h, hsh))
...
runChainweb cw = do
  ...
  where
    ...
    requestSizeLimit =
      requestSizeLimitMiddleware $
        setMaxLengthForRequest
          (\_req -> pure $ Just $ 2 * 1024 * 1024) -- 2MB
    ...
    serviceApiServerSettings port interface =
      defaultSettings
        & setPort (int port)
        & setHost interface
        & setOnException
