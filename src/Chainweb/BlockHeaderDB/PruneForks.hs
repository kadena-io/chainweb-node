      ...
      | int (_blockHeight hdr) <= depth -> do
          logg Info $
            "Skipping database pruning because the maximum block height "
              ...
              <> " is not larger than then requested depth "
              <> sshow depth
          return 0
      | int (_blockHeight hdr) <= int genHeight + depth -> do
          logg Info $
            "Skipping database pruning because there are not yet"
...
pruneForks_ logg cdb mar mir callback = do
  logg Debug $
    "Pruning block header database for chain "
      <> sshow (_chainId cdb)
      <> " with upper bound "
      <> sshow (_getMaxRank mar)
      <> " and lower bound "
      <> sshow (_getMinRank mir)

  -- parent hashes of all blocks at height max rank @mar@.
  --
  !pivots <-
    entries cdb Nothing Nothing (Just $ MinRank $ Min $ _getMaxRank mar) (Just mar) $
  ...
  where
          ...
          throwM $
            InternalInvariantViolation $
              "PruneForks.pruneForks_: detected a corrupted database. Some block headers are missing"
                ...
                <> encodeToText pivots
                <> ". Current header: "
                <> encodeToText (ObjectEncoded cur)
                <> ". Previous height: "
                <> sshow prevHeight
    ...
    deleteHdr k = do
      ...
      logg Debug $
        "pruned block header "
          <> encodeToText (_blockHash k)
          <> " at height "
          <> sshow (_blockHeight k)
