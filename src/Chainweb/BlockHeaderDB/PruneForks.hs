    ...
    pruneBlocks initialState =
        ...
        go state strm =
            ...
            then do
                ...
                -- we want to resume a level higher, because we really don't
                -- prune the first height, we use it for the initial live set
                tableInsert (_webCurrentPruneJob wbhdb) () (succ $ prevHeight state, startedFrom pruneJob)
                go
