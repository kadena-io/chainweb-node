...
instance HasVersion => TreeDb RemoteDb where
    ...
    keys (RemoteDb env alog cid) next limit minr maxr f
      ...
      where
        ...
        client nxt = logServantError alog "failed to query tree db keys"
            $ hashesClient cid limit nxt minr maxr

    entries (RemoteDb env alog cid) next limit minr maxr f
        = f $ callAndPage client next 0 env
      where
        ...
        client nxt = logServantError alog "failed to query tree db entries"
            $ headersClient cid limit nxt minr maxr

    branchKeys (RemoteDb env alog cid) next limit minr maxr lower upper f
        = f $ callAndPage client next 0 env
      where
        client :: Maybe (NextItem BlockHash) -> ClientM (Page (NextItem BlockHash) BlockHash)
        client nxt = logServantError alog "failed to query remote branch keys"
            $ branchHashesClient cid limit nxt minr maxr (BranchBounds lower upper)

    branchEntries (RemoteDb env alog cid) next limit minr maxr lower upper f
        = f $ callAndPage client next 0 env
      where
        client :: Maybe (NextItem BlockHash) -> ClientM (Page (NextItem BlockHash) BlockHeader)
        client nxt = logServantError alog "failed to query remote branch entries"
            $ branchHeadersClient cid limit nxt minr maxr (BranchBounds lower upper)

    -- We could either use the cut or create a new API
    -- maxEntry (RemoteDb env alog cid) e =
...
remoteDb db logg env = do
    h <- root db
    pure $! RemoteDb env (ALogFunction logg) (view blockChainId h)
