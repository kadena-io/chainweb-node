...
import Chainweb.Pact.Backend.Types (BlockHandle, PactDbFor, emptyBlockHandle, pendingSuccessfulTxs, blockHandlePending)
...
import Chainweb.Pact.Service.Types (throwIfNoHistory)
...
runPactBlockM
  :: ParentHeader -> PactDbFor logger Pact5
...
runPactBlockM pctx dbEnv (PactBlockM r) = PactServiceM $ ReaderT $ \e -> StateT $ \s -> do
  startHandle <- throwIfNoHistory =<< startBlock dbEnv (Just pctx)
  (r, s') <- runStateT
    (runReaderT r (PactBlockEnv e pctx dbEnv))
    (PactBlockState s startHandle)
...
registerProcessedTx :: Hash -> PactBlockM logger tbl ()
registerProcessedTx hsh =
  pbBlockHandle . blockHandlePending . pendingSuccessfulTxs <>=
    HashSet.singleton (SB.fromShort $ unHash hsh)

pactTransaction :: (PactDb Pact5.CoreBuiltin Pact5.Info -> IO a) -> PactBlockM logger tbl a
pactTransaction k = do
  e <- view psBlockDbEnv
  h <- use pbBlockHandle
  (r, h') <- liftIO $ doPact5DbTransaction e h k
  pbBlockHandle .= h'
  return r
...
catchesPactError :: (MonadCatch m, MonadIO m, Logger logger) => logger -> m a -> m (Either (Pact5.PactError Pact5.Info) a)
catchesPactError logger action = catches (Right <$> action)
