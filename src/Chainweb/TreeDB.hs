...
instance (Show (DbEntry db), Show (DbKey db)) => Show (TreeDbException db) where
  ...
  show (TreeDbAncestorMissing e r msg) =
    "TreeDbAncestorMissing: entry "
      ++ show e
      ++ ". Rank "
      ++ sshow r
      ++ ". "
...
instance
  ...
  where
  parseJSON = withObject "BranchBounds" $ \o ->
    BranchBounds
...
lookupRankedM ::
  ...
  DbKey db ->
  IO (DbEntry db)
lookupRankedM db r k =
  lookupRanked db r k >>= \case
    Nothing -> throwM $ TreeDbKeyNotFound @db k
    (Just !x) -> return x
          ...
          !bs <-
            S.toList_
              & defaultBranchEntries
                ...
                (Just $ int r)
                mempty
                (HS.fromList $ UpperBound . key <$> as)

          -- check that result is unique
          case bs of
            [] ->
              throwM $
                TreeDbAncestorMissing @db h (int r) $
...
ancestorOfEntry ::
  ...
  DbEntry db ->
  IO Bool
ancestorOfEntry db h ctx =
  lookup db h >>= \case
    Nothing -> return False
    Just lh ->
      seekAncestor db ctx (rank lh) >>= \case
        Nothing -> return False
