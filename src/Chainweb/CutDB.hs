  ...
  where
    ...
    go it =
      iterValue it >>= \case
        ...
        Just ch ->
          try (lookupCutHashes wbhdb ch) >>= \case
            Left (e@(TreeDbKeyNotFound _) :: TreeDbException BlockHeaderDb) -> do
              logfun @T.Text Warn $
                "Unable to load cut at height "
                  <> sshow (_cutHashesHeight ch)
                  <> " from database."
                  <> " Error: "
                  <> sshow e
                  <> "."
...
processCuts conf logFun headerStore payloadStore cutHashesStore queue cutVar = do
  ...
  queueToStream
    ...
    & S.mapM_
      ( \newCut -> do
          ...
          !resultCut <-
            trace logFun "Chainweb.CutDB.processCuts._joinIntoHeavier" () 1 $
  ...
  where
    ...
    farAhead x = do
      ...
      when r $
        loggc Debug x $
          "skip far ahead cut. Current maximum block height: "
            <> sshow curMax
            <> ", got: "
            <> sshow newMax
  ...
  where
    ...
    go =
      trace logfun "Chainweb.CutDB.cutHashesToBlockHeaderMap" hsid 1 $ do
        ...
        (headers :> missing) <-
          S.each (HM.toList $ _cutHashes hs)
...
memberOfHeader db cid h ctx = do
  lookup chainDb h >>= \case
    ...
    Just lh ->
      seekAncestor chainDb ctx (int $ _blockHeight lh) >>= \case
        Nothing -> return False
