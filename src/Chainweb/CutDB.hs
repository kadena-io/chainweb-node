...
cutHashesToBlockHeaderMap conf logfun headerStore providers hs =
  ...
  where
    ...
    go = do
        ...
        -- FIXME: pass candidate payloads to payload provider
        -- This can be done via the evaluation ctx
        -- plds <- emptyTable
        ...
        (headers :> missing) <- S.each (HM.toList $ _cutHashes hs)
            ...
            & S.mapM (tryGetBlockHeader hdrs localPayload)
            & S.partitionEithers
    ...
    tryGetBlockHeader hdrs localPayload cv@(cid, _) = do
