...
chainweb218Pact
    ...
    -> BlockHeight
    -> Bool
chainweb218Pact aoa v h = case aoa of
    At -> go (==) v h
    After -> go (<) v h
  where
    go f Mainnet01 = f $ error "TODO"
    go f Testnet04 = f $ error "TODO"
    go f Development = f 480
    go f (FastTimedCPM g) | g == petersonChainGraph = f 60
    go f _ = f 25
