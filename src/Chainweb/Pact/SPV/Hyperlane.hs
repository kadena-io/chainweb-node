...
hyperlaneAnnouncementVerifierPlugin :: VerifierPlugin
hyperlaneAnnouncementVerifierPlugin = VerifierPlugin $ \vals caps gasRef -> do
  -- extract capability values
  let SigCapability{..} = Set.elemAt 0 caps
  ...
  let addressVal = PLiteral . LString . Text.decodeUtf8 <$> address
