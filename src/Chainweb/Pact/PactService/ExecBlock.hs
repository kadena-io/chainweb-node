...
import Pact.Types.ExpParser (ParseEnv (..), mkTextInfo)
...
execBlock currHeader plData pdbenv = do
  ...
  --
  txValidationTime <-
    if isGenesisBlockHeader currHeader
  ...
  asks _psValidateHashesOnReplay >>= \x ->
    when x $
...
applyPactCmd isGenesis env miner txTimeLimit cmd = StateT $ \(T2 mcache maybeBlockGasRemaining) -> do
  ...
  v <- view psVersion
  let onBuyGasFailure (BuyGasFailure f) = pure $! (Left f, T2 mcache maybeBlockGasRemaining)
    ...
    case maybeBlockGasRemaining of
      Just blockGasRemaining ->
        when (P._crGas result >= succ blockGasRemaining) $
          ...
          throwM $
            BlockGasLimitExceeded (blockGasRemaining - fromIntegral requestedTxGasLimit)
      Nothing -> return ()
...
transactionsFromPayload chainCtx plData = do
  ...
  unless (null theLefts) $ do
    ...
    throwM $
      TransactionDecodeFailure $
        "Failed to decode pact transactions: "
          <> T.intercalate ". " ls
  ...
  where
    toCWTransaction bs =
      evaluate
        ( force
            ( codecDecode (chainwebPayloadCodec chainCtx) $
...
validateHashes bHeader pData miner transactions =
  ...
  where
    ...
    toPairCR cr =
      over
        (P.crLogs . _Just)
        (CRLogPair (fromJuste $ P._crLogs (toHashCommandResult cr)))
        cr
    ...
    details =
      concat
        [ check "Miner" [] prevMiner newMiner,
          check
            "TransactionsHash"
            (checkTransactions prevTransactions newTransactions)
            prevTransactionsHash
            newTransactionsHash,
          check
            "OutputsHash"
            (addOutputs transactions)
            prevOutputsHash
            newOutputsHash
        ]
