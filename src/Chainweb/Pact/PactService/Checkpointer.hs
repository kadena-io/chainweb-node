...
readFromNthParent n doRead = do
    ...
    withSavepoint sql ReadFromNSavepoint $ do
        ...
        then return NoHistory
            ...
            maybeNthBlock <- lookupBlock
                (Right (_syncStateHeight latest - fromIntegral @Word @BlockHeight n))
            case maybeNthBlock of
                -- this case for shallow nodes without enough history
                Nothing -> return NoHistory
                Just nthBlock -> readFrom (Parent nthBlock) doRead
...
lookupBlock :: Either BlockHash BlockHeight -> PactServiceM logger tbl (Maybe RankedBlockHash)
lookupBlock b = do
    sql <- view psReadWriteSql
    liftIO $ ChainwebPactDb.throwOnDbError $ ChainwebPactDb.lookupBlock sql b

lookupBlockRanked :: RankedBlockHash -> PactServiceM logger tbl Bool
lookupBlockRanked b = do
    sql <- view psReadWriteSql
    liftIO $ ChainwebPactDb.throwOnDbError $ ChainwebPactDb.lookupBlockRanked sql b
