...
import Chainweb.TreeDB (getBranchIncreasing, forkEntry, lookup)
...
readFromLatest doRead = do
    ...
    readFrom (Just $ parentToParentContext latestBlockHeader) doRead

readFrom :: Maybe ParentContext -> PactBlockM logger tbl a -> PactServiceM logger tbl a
readFrom pc doRead = do
    cp <- getCheckpointer
    v <- view chainwebVersion
    cid <- view chainId
    _cpReadFrom (_cpReadCp cp) pc $
        (\dbenv -> runPactBlockM (fromMaybe (genesisParentContext v cid) pc) dbenv doRead)
...
-- exception is raised.
--
-- TODO: why exactly is Nothing even allowed as a parameter here?
rewindToIncremental
    :: forall logger tbl
    . (HasCallStack, CanReadablePayloadCas tbl, Logger logger)
    => Maybe RewindLimit
                ...
                let playChunk :: IORef BlockHeight -> BlockHeader -> Stream (Of BlockHeader) IO r -> IO (Of BlockHeader r)
                    playChunk heightRef cur s = do
                        (r, Last header) <- _cpRewindAndExtend
                            cp
                            (Just $ parentToParentContext $ ParentHeader cur)
                            (parentToParentContext . ParentHeader)
                            s
                            $ \dbEnv pc bh -> do
