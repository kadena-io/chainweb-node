...
data SomeBlockM logger tbl a where
    Pact4BlockM :: Pact4.PactBlockM logger tbl a -> SomeBlockM logger tbl a
    Pact5BlockM :: Pact5.PactBlockM logger tbl a -> SomeBlockM logger tbl a
...
readFrom ph doRead = do
    ...
    case doRead of
        Pact4BlockM act ->
            liftIO $ _cpReadFrom (_cpReadCp cp) ph Pact4T $ \dbenv ->
                evalPactServiceM s e $
                    Pact4.runPactBlockM pactParent dbenv act

        Pact5BlockM act ->
            liftIO $ _cpReadFrom (_cpReadCp cp) ph Pact5T $ \dbenv ->
                evalPactServiceM s e $ do
                    fst <$> Pact5.runPactBlockM pactParent dbenv act
            ...
            $ blocks & S.map (\case
                Pact4BlockM block ->
                    Pact4RunnableBlock $ \dbEnv mph -> runPact $ do
                        ...
                        Pact4.runPactBlockM pactParent dbEnv block
                Pact5BlockM block ->
                    Pact5RunnableBlock $ \dbEnv mph -> runPact $ do
                        pactParent <- getPactParent mph
                        ((q, header), handle) <- Pact5.runPactBlockM pactParent dbEnv block
                        return (q, header, handle)
            )
