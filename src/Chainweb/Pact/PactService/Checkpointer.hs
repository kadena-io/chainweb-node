...
import Chainweb.TreeDB (forkEntry, getBranchIncreasing, lookup, lookupM)
...
withCheckpointerWithoutRewind target caller act = do
  ...
  unlessM (asks _psIsBatch) $
    error $
      "Code invariant violation: withCheckpointerRewind called by " <> caller <> " outside of batch. Please report this as a bug."
...
rewindTo rewindLimit (Just (ParentHeader parent)) = do
  ...
  (_, lastHash) <-
    getCheckpointer >>= liftIO . _cpGetLatestBlock >>= \case
      Nothing -> throwM NoBlockValidatedYet
      Just p -> return p

  if lastHash == parentHash
    then -- We want to guarantee that '_psParentHeader' is in sync with the
    ...
    -- 'withCheckPointerWithoutRewind'.
      ...
      logInfo $
        T.unpack $
          "rewind from last to checkpointer target"
            <> ". last height: "
            <> sshow (_blockHeight lastHeader)
            <> "; last hash: "
            <> blockHashToText (_blockHash lastHeader)
            <> "; target height: "
            <> sshow parentHeight
            <> "; target hash: "
            <> blockHashToText parentHash
  ...
  where
    ...
    playFork lastHeader = do
      ...
      if commonAncestor == parent
        then -- If no blocks got replayed the checkpointer isn't restored via
        ...
        withCheckpointerWithoutRewind (Just $ ParentHeader commonAncestor) "rewindTo" $ \_ ->
          ...
          logInfo $
            "rewindTo.playFork"
              <> ": checkpointer is at height: "
              <> sshow (_blockHeight lastHeader)
              <> ", target height: "
              <> sshow (_blockHeight parent)
              <> ", common ancestor height "
              <> sshow ancestorHeight
...
fastForward (target, block) =
  ...
  withCheckpointerWithoutRewind (Just target) "fastForward" $ \pdbenv -> do
    ...
    payload <-
      liftIO $
        tableLookup payloadDb bpHash >>= \case
          Nothing ->
            throwM $
              PactInternalError $
                "Checkpointer.rewindTo.fastForward: lookup of payload failed"
                  ...
                  <> encodeToText bpHash
                  <> ". Block: "
                  <> encodeToText (ObjectEncoded block)
        ...
        logInfo $
          T.unpack $
            T.pack caller
              <> ".syncParentHeader"
              <> "; current hash: "
              <> blockHashToText (_blockHash cur)
              <> "; current height: "
              <> sshow (_blockHeight cur)
              <> "; checkpointer hash: "
              <> blockHashToText ph
              <> "; checkpointer height: "
              <> sshow h
          ...
          (_ S.:> c) <- withPactState $ \runPact ->
            getBranchIncreasing bhdb parent (int ancestorHeight) $ \newBlocks -> do
              ...
              let playChunk :: IORef BlockHeight -> BlockHeader -> Stream (Of BlockHeader) IO r -> IO (Of BlockHeader r)
                  playChunk heightRef cur s = withBatchIO runPact $ \runPactLocal ->
                    S.foldM
