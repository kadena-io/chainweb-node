...
ctxToPublicData (TxContext parentContext pm) = PublicData
  ...
  where
    BlockHeight bh = _parentContextCurrentHeight parentContext
...
ctxToPublicData' (TxContext parentContext pm) = PublicData
  ...
  where
    ...
    BlockCreationTime (Time (TimeSpan (Micros !bt))) =
      _parentContextCreationTime parentContext
    BlockHash h = _parentContextHash parentContext

-- | Retrieve parent context as 'ParentContext'
ctxParentContext :: TxContext -> ParentContext
ctxParentContext (TxContext pc _) = pc

-- | Get "current" block height, which means parent height + 1 outside of
-- genesis.
...
ctxCurrentBlockHeight = _parentContextCurrentHeight . ctxParentContext
...
liftPactServiceM (PactServiceM a) = PactBlockM (magnify pactServiceEnv a)
...
getInitCache = do
  ph <- views pactParentContext _parentContextHeight
