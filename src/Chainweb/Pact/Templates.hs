...
mkBuyGasTerm ::
  -- | Id of the miner to fund
  MinerId ->
  -- | Miner keyset
  MinerKeys ->
  -- | Address of the sender from the command
  Text ->
  -- | The gas limit total * price
  GasSupply ->
  (Term Name, ExecMsg ParsedCode)
mkBuyGasTerm (MinerId mid) (MinerKeys ks) sender total = (populatedTerm, execMsg)
  where
    (term, senderS, minerS) = buyGasTemplate
