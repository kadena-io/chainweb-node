...
applyCmd v logger pdbenv miner gasModel txCtx spv cmdIn mcache0 =
  ...
  where
    ...
    listErrMsg = "Unknown primitive \"list\" in determining cost of GUnreduced CallStack (from HasCallStack):"
    ...
    isOldListErr = \case
      PactError EvalError _ _ doc -> renderCompactText' doc == renderCompactText' listErrMsg
      _ -> False
