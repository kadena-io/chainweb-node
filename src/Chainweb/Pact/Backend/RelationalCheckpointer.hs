          ...
          let
            ...
            mkBlockDbEnv db = PactDb.CurrentBlockDbEnv
              ...
              , PactDb._cpRegisterProcessedTx = \hash ->
                PactDb.runBlockEnv newDbEnv (PactDb.indexPactTransaction $ BS.fromShort hash)
              , PactDb._cpLookupProcessedTx = \hs ->
                PactDb.runBlockEnv newDbEnv (PactDb.callDb "lol" $ \sql -> doLookupSuccessful sql currentHeight hs)
              }
            pactDb
              | parentIsLatestHeader = PactDb.chainwebPactDb
              | otherwise = PactDb.rewoundPactDb currentHeight startTxId
          doRead (mkBlockDbEnv pactDb)
              ...
              let
                ...
                mkBlockDbEnv db = PactDb.CurrentBlockDbEnv
                  ...
                  , PactDb._cpLookupProcessedTx = \hs -> PactDb.runBlockEnv dbMVar $
                      PactDb.callDb "something" $ \sql -> doLookupSuccessful sql bh hs
                  }
          ...
          Pact5RunnableBlock runBlock
            | pact5 v cid bh -> do
              let
                ...
                pactDb
                  = Pact5.chainwebPactCoreBlockDb Nothing Pact5.Transactional blockEnv
              (q, blockHeader, blockHandle) <- runBlock pactDb maybeParent
              undefined
...
doLookupSuccessful :: Database -> BlockHeight -> V.Vector BS.ShortByteString -> IO (HashMap.HashMap BS.ShortByteString (T2 BlockHeight BlockHash))
doLookupSuccessful db curHeight hashes = do
  fmap buildResultMap $ do -- swizzle results of query into a HashMap
      let
        ...
        qvals
          ...
          = map (\h -> SBlob $ BS.fromShort h) hss
  ...
  where
    ...
    buildResultMap :: [T3 BS.ShortByteString BlockHeight BlockHash] -> HashMap.HashMap BS.ShortByteString (T2 BlockHeight BlockHash)
    buildResultMap xs = HashMap.fromList $
      map (\(T3 txhash blockheight blockhash) -> (txhash, T2 blockheight blockhash)) xs

    go :: [SType] -> IO (T3 BS.ShortByteString BlockHeight BlockHash)
