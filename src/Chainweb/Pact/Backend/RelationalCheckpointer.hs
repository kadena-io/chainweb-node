...
doRestore _loggr v cid dbenv (Just (bh, hash)) = runBlockEnv dbenv $ do
  setModuleNameFix
  setSortedKeys
  setLowerCaseTables
  clearPendingTxState
...
doSave dbenv hash = runBlockEnv dbenv $ do
  ...
  where
    ...
    runPending bh = do
      ...
      log "doSave.runPending: indexPendingPactTransactions"
    ...
    toVectorChunks writes = liftIO $ do
      mv <-
        mutableVectorFromList . DL.toList . DL.concat $
...
doGetLatest dbenv =
  ...
  where
    qtext =
      "SELECT blockheight, hash FROM BlockHistory \
...
doLookupBlock dbenv (bheight, bhash) = runBlockEnv dbenv $ do
  r <- callDb "lookupBlock" $ \db ->
    qry
      db
      qtext
      [SInt $ fromIntegral bheight, SBlob (runPutS (encodeBlockHash bhash))]
      [RInt]
  ...
  where
    qtext =
      "SELECT COUNT(*) FROM BlockHistory WHERE blockheight = ? \
      \ AND hash = ?;"
...
doGetBlockHistory dbenv blockHeader d = runBlockEnv dbenv $ do
  ...
  where
    ...
    queryHistory db tableName s e = do
      let sql =
            "SELECT txid, rowkey, rowdata FROM [" <> tableName
              <> "] WHERE txid >= ? AND txid < ?"
      r <-
        qry
          db
...
doGetHistoricalLookup dbenv blockHeader d k = runBlockEnv dbenv $ do
  ...
  where
    ...
    queryHistoryLookup db tableName e rowKeyName = do
      let sql =
            "SELECT rowKey, rowdata FROM [" <> tableName
...
log :: String -> BlockHandler SQLiteEnv ()
log msg = do
  BlockDbEnv _ loggr <- ask
  logError_ loggr msg
