...
chainwebPactCoreDb :: (Logger logger) => MVar (BlockEnv logger SQLiteEnv) -> PactDb CoreBuiltin ()
...
rewoundPactCoreDb :: (Logger logger) => MVar (BlockEnv logger SQLiteEnv) -> BlockHeight -> TxId -> PactDb CoreBuiltin ()
...
doReadRow mlim d k = forModuleNameFix $ \mnFix ->
  ...
  where
    tableName = domainTableNameCore d
    ...
    lookupInDb
        ...
        -> (Utf8 -> BS.ByteString -> MaybeT (BlockHandler logger SQLiteEnv) v)
        -> MaybeT (BlockHandler logger SQLiteEnv) v
    lookupInDb rowkey f checkCache = do
...
writeSys d k v = gets _bsTxId >>= go
  where
    go txid = do
        (kk, vv) <- use bsModuleNameFix >>= \mnFix -> pure $ case d of
  ...
  where
    ...
    go (Pact4.TxId txid) = do
        ...
        row' <- case m of
                    Nothing -> ins
                    (Just old) -> upd old
        recordTxLog (Pact4.TableName tn) d (convRowKeyCore k) (_encodeRowData serialisePact rowdata)
...
failIfTableDoesNotExistInDbAtHeight
  :: T.Text -> Utf8 -> BlockHeight -> BlockHandler logger SQLiteEnv ()
...
doCommit :: BlockHandler logger SQLiteEnv [TxLog B8.ByteString]
...
doGetTxLog :: TableName -> TxId -> BlockHandler logger SQLiteEnv [TxLog RowData]
