...
bsPendingTxOrError :: HasCallStack => Lens' BlockState (SQLitePendingData, DList (Pact5.TxLog ByteString))
bsPendingTxOrError = lens
    (\bs -> case _bsPendingTx bs of
        Nothing -> error "bsPendingTxOrError: not in tx"
        Just t -> t
    )
...
data Pact5Db = Pact5Db
    { doPact5DbTransaction
        ...
        -> IO (a, BlockHandle)
    , startBlock :: Maybe ParentHeader -> IO (Historical BlockHandle)
    }
...
chainwebPactCoreBlockDb maybeLimit mode env = Pact5Db
    { doPact5DbTransaction = \blockHandle kont -> do
        ...
        when (isNothing (_bsPendingTx finalState)) $
            internalError "dangling transaction"

        return (r, _bsBlockHandle finalState)
    , startBlock = \ph -> do
        getEndTxId "startBlock" (_blockHandlerDb env) ph <&>
            fmap \(TxId txid) ->
                emptyBlockHandle $ Pact4.TxId txid
    }
...
getPendingData = do
    ...
    s <- get
    let ptx = s ^. (bsPendingTxOrError . _1)
...
recordTxLog tn d (Utf8 k) v = do
    ...
    (pendingSQLite, txlogs) <- use bsPendingTxOrError
...
modifyPendingData f = do
    (pending, txlogs) <- use bsPendingTxOrError
...
doCommit = view blockHandlerMode >>= \case
    m -> do
        ...
        then do
            ...
            pending <- use bsPendingTxOrError
...
doBegin m = do
    ...
    use bsPendingTx >>= \case
        Just _ -> do
            error "CorePactDb.beginTx: In transaction already"
        Nothing -> do
            modify'
