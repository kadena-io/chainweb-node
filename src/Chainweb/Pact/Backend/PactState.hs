...
bulkInsert :: Database -> Text -> [[SType]] -> IO ()
bulkInsert db tblname values = do
  let oneRow rowSize = "(" <> Text.intercalate ", " (List.replicate rowSize "?") <> ")"
  let allRows rows = Text.intercalate ", " (List.map (\row -> oneRow (List.length row)) rows)
  case values of
    [] -> do
      pure ()
    rows -> do
      let q = toUtf8 $ "INSERT INTO [" <> tblname <> "] VALUES " <> allRows rows
      bracket (SQL.prepareUtf8 db q) Direct.finalize $ \s -> do
        forM_ (List.zip [1..] (List.concat values)) $ \(argIndex, arg) -> do
          case arg of
            SInt a -> Direct.bindInt64 s argIndex a
            SDouble a -> Direct.bindDouble s argIndex a
            SText a -> Direct.bindText s argIndex a
            SBlob a -> Direct.bindBlob s argIndex a
        void $ SQL.stepNoCB s `finally` (SQL.clearBindings s >> SQL.reset s)
