...
import Chainweb.Version (ChainId, ChainwebVersion, genesisHeight)
...
import qualified Database.SQLite.Simple as Simpl
...
doReadRow d k = forModuleNameFix $ \mnFix ->
  ...
  where
    ...
    lookupInPendingData (Utf8 rowkey) p = do
      ...
      if null ddata
        then -- should be impossible, but we'll check this case
          mzero
        else -- we merge with (++) which should produce txids most-recent-first
        -- -- we care about the most recent update to this rowkey
          MaybeT $ return $! decodeStrict' $ DL.head ddata
...
backendWriteUpdateBatch ::
  BlockHeight ->
  -- | updates chunked on table name
  [(Utf8, V.Vector SQLiteRowDelta)] ->
  Database ->
  IO ()
...
toTxLog ::
  (MonadThrow m, FromJSON v, FromJSON l) =>
  Domain k v ->
  Utf8 ->
  BS.ByteString ->
  m (TxLog l)
  ...
  where
    ...
    dbIndexTransactions txs = do
      ...
      when (length txs' > 0) $ do
        logError $ "dbIndexTransactions.blockheight = " <> show bh
        logError $ "dbIndexTransactions.hashes = " <> show (map B8.unpack (toList txs))
        duplicates :: [(Word, B8.ByteString)] <- callDb "dbIndexTransactions.debugging" $ \db -> do
          let qryText = fromString $ "SELECT blockheight, txhash FROM TransactionIndex WHERE txhash IN " ++ fmtQryArray (length txs') ++ " ORDER BY blockheight ASC;"
          Simpl.query (Simpl.Connection db) qryText txs'
        if (length duplicates > 0)
          then do
            logError $
              ("dbIndexTransactions: duplicate txhashes: " <>) $
                show $
                  flip map duplicates $ \(blockheight, txhash) ->
                    "blockheight = " ++ show blockheight ++ ", txhash = " ++ B8.unpack txhash
          else do
            logError $ "dbIndexTransactions: no duplicate hashes"
      callDb "dbIndexTransactions" $ \db -> do
        execMulti
          db
          "INSERT INTO TransactionIndex (txhash, blockheight) \
          \ VALUES (?, ?)"
          rows
...
clearTxIndex = do
  ...
  callDb "clearTxIndex" $ \db -> do
    exec'
      db
      "DELETE FROM TransactionIndex WHERE blockheight >= ?;"
      [SInt (fromIntegral bh)]

createBlockHistoryTable :: BlockHandler SQLiteEnv ()
createBlockHistoryTable =
  callDb "createBlockHistoryTable" $ \db ->
    exec_
      db
...
createTableCreationTable =
  callDb "createTableCreationTable" $ \db ->
    exec_
      db
  ...
  where
    ...
    getBCurrentHeight = do
      r <- callDb "handlePossibleRewind" $ \db ->
        qry_
          db
          "SELECT max(blockheight) AS current_block_height \
          \FROM BlockHistory;"
          [RInt]
      bh <-
        liftIO $
          expectSingleRowCol "handlePossibleRewind: (block):" r >>= \case
            SInt x -> return x
    ...
    checkHistoryInvariant succOfCurrent = do
      ...
      where
        ...
        missingBlockMessage =
          T.pack $
            printf
              "handlePossibleRewind: The checkpointer attempted to restore to block hash\
              \ %s at block height %d, which is not in the current block history of the\
              \ checkpointer at height %d."
              (blockHashToText hsh)
              (_height bRestore - 1)
              (_height succOfCurrent - 1)

        rowCountErrorMessage =
          T.pack
            . printf
              "At this blockheight/blockhash (%d, %s) in BlockHistoryTable, there are %d entries."
              (fromIntegral bRestore :: Int)
              (show hsh)

        futureRestorePointMessage :: Text
        futureRestorePointMessage =
          T.pack $
            printf
              "handlePossibleRewind: The checkpointer attempted to restore to block hash %s\
    ...
    newChildBlock bCurrent = do
      ...
      where
        msg = "handlePossibleRewind: newChildBlock: error finding txid"
...
vacuumTablesAtRewind bh endingtx droppedtbls db = do
  ...
  forM_ toVacuumTblNames $ \tblname ->
    exec'
      db
      ("DELETE FROM " <> tbl (Utf8 tblname) <> " WHERE txid >= ?")
      [SInt $! fromIntegral endingtx]
  exec'
    db
    "DELETE FROM VersionedTableMutation WHERE blockheight >= ?;"
    [SInt (fromIntegral bh)]
...
deleteHistory bh = do
  callDb "Deleting from BlockHistory, VersionHistory" $ \db -> do
    exec'
      db
      "DELETE FROM BlockHistory WHERE blockheight >= ?"
      [SInt (fromIntegral bh)]
...
fmtQryArray :: Int -> String
fmtQryArray n = '(' : (List.intersperse ',' (replicate n '?') ++ ")")
