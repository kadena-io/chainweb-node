...
tableExists :: Utf8 -> BlockHeight -> BlockHandler logger SQLiteEnv Bool
tableExists tableName bh = do
    let knownTbls = join
  ...
  where
    ...
    lookupInDb rowkey checkCache = do
        ...
        case mlim of
            Nothing -> return ()
            Just (bh, _) -> do
                existsInDb <- lift $ tableExists tableName bh
                unless existsInDb $
