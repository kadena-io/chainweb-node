...
open_v2 :: Utf8 -> Word32 -> Utf8 -> IO (Either (Error, Utf8) Database)
open_v2 (Utf8 path) flag (Utf8 vfs_module) =
  BS.useAsCString vfs_module $ \vfs_module' ->
    BS.useAsCString path $ \path' ->
      alloca $ \database -> do
          rc <- c_sqlite3_open_v2 path' database (fromIntegral flag) vfs_module'
