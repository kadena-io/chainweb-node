...
newtype PactServerData_ (v :: ChainwebVersionT) (c :: ChainIdT) logger tbl = PactServerData_ {_unPactServerData :: PactServerData logger tbl}

data SomePactServerData
  = forall v c logger tbl.
...
spvHandler l cdb cid (SpvRequest rk (Pact.ChainId ptid)) = do
  ...
  T2 bhe _bha <-
    liftIO (_pactLookup pe (NoRewind cid) (pure ph)) >>= \case
      Left e ->
  ...
  idx <-
    liftIO (getTxIdx bdb pdb bhe ph) >>= \case
      Left e ->
        toErr $
  ...
  p <-
    liftIO (try $ createTransactionOutputProof cdb tid cid bhe idx) >>= \case
...
internalPoll pdb bhdb mempool pactEx cut requestKeys0 = do
  ...
  where
    ...
    checkBadList rkeys = do
      ...
      let bad =
            V.map (RequestKey . Hash . unTransactionHash . fst) $
              V.filter snd $
                V.zip hashes out
