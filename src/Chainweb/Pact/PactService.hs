    ...
    case latestBlock of
      ...
      Just currentBlockHeader -> do
        ...
        then do
          ...
          !mc <- liftIO $ _cpReadFrom (_cpReadCp cp) pc $ \pdbenv -> do
...
execNewBlock mpAccess miner = do
    ...
    T2 latestHeader <$> execNewBlock' mpAccess latestHeader miner
...
execNewBlock' mpAccess latestHeader miner = pactLabel "execNewBlock" $ do
    updateMempool
    readFrom (parentToParentContext latestHeader) $ do
        liftPactBlockM $
...
execNewGenesisBlock miner newTrans = pactLabel "execNewGenesisBlock" $ do
    ...
    cid <- view chainId
    readFrom (GenesisParentContext v cid) $ do
...
execLocal cwtx preflight sigVerify rdepth = pactLabel "execLocal" $ do
    ...
    readFrom (parentToParentContext rewindHeader) $ do
        ctx <- getTxContext pm
...
execValidateBlock memPoolAccess headerToValidate payloadToValidate = pactLabel "execValidateBlock" $ do
    ...
    logBlockHash $ do
        rewindTarget <- liftIO $ case (currHeader, parentOfHeaderToValidate) of
            (Just currHeader', Just ph) ->
                Just <$> forkEntry bhdb currHeader' (_parentHeader ph)
            _ ->
                return Nothing
        -- for the purpose of the rewind limit check, pre-genesis and genesis
        -- are at the same height.
        let !currHeight = maybe (genesisHeight v cid) _blockHeight currHeader
        let !ancestorHeight = maybe (genesisHeight v cid) _blockHeight rewindTarget
        ...
        unless rewindLimitSatisfied $
            throwM $ RewindLimitExceeded
                ...
                currHeader
                rewindTarget
        withForkStream <- case (rewindTarget, parentOfHeaderToValidate) of
            (Just commonAncestor, Just ph) ->
                return (getBranchIncreasing bhdb (_parentHeader ph) (int $ _blockHeight commonAncestor + 1))
            (Nothing, Just ph) ->
                return (getBranchIncreasing bhdb (_parentHeader ph) (int $ genesisHeight v cid))
            (_, Nothing) ->
        ...
        (numBlocksExecuted, results) <- withPactState $ \runPact -> do
            ...
            withForkStream $ \newBlocks -> do
                ...
                _cpRewindAndExtend
                    cp
                    (maybe (genesisParentContext v cid) (parentToParentContext . ParentHeader) rewindTarget)
                    ...
                    $ \dbEnv pc -> \case
                      ...
                      Just forkBh -> do
                          ...
                          let pc' = parentToParentContext (ParentHeader forkBh)
                          return ([], pc')
                      ...
                      Nothing -> do
                        ...
                        let pc' = parentToParentContext (ParentHeader headerToValidate)
                        return ([(result, totalGasUsed)], pc')
