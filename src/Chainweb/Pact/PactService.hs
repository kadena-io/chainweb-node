...
execValidateBlock memPoolAccess headerToValidate payloadToValidate = pactLabel "execValidateBlock" $ do
    ...
    logBlockHash $ do
        ...
        withForkStream <- case (rewindTarget, parentOfHeaderToValidate) of
            (Just commonAncestor, Just ph) -> do
                -- note the + 1 here because we don't want to execute
                -- commonAncestor, we want to rewind to commonAncestor and
                -- execute its child on the fork we're playing
                let firstForkBlockHeight = int $ _blockHeight commonAncestor + 1
                return (getBranchIncreasing bhdb (_parentHeader ph) firstForkBlockHeight)
            (Nothing, Just ph) ->
