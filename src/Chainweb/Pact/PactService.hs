...
initializeLatestBlock unlimitedRewind =
  ...
  where
    go = do
      ...
      case msg of
        ...
        NewBlockMsg NewBlockReq {..} -> do
          trace
            logFn
            "Chainweb.Pact.PactService.execNewBlock"
            (_parentHeader _newBlockHeader)
            1
        ...
        ValidateBlockMsg ValidateBlockReq {..} -> do
          trace
            logFn
            "Chainweb.Pact.PactService.execValidateBlock"
            _valBlockHeader
        ...
        LookupPactTxsMsg (LookupPactTxsReq restorePoint txHashes resultVar) -> do
          trace
            logFn
            "Chainweb.Pact.PactService.execLookupPactTxs"
            ()
            (length txHashes)
        ...
        PreInsertCheckMsg (PreInsertCheckReq txs resultVar) -> do
          trace
            logFn
            "Chainweb.Pact.PactService.execPreInsertCheckReq"
            ()
            (length txs)
    ...
    tryOne' which mvar post m =
      (evalPactOnThread (post <$> m) >>= (liftIO . putMVar mvar))
        `catches` [ Handler $ \(e :: SomeAsyncException) -> do
                      logWarn $
                        mconcat
  ...
  where
    ...
    refill fetchLimit txTimeLimit pdbenv unchanged@(BlockFilling bfState oldPairs oldFails) = do
      ...
      when (_bfCount bfState > fetchLimit) $
        throwM $
          MempoolFillFailure $
            "Refill fetch limit exceeded (" <> sshow fetchLimit <> ")"

      when (_bfGasLimit bfState < 0) $
        throwM $
          MempoolFillFailure $
            "Internal error, negative gas limit: " <> sshow bfState

      if _bfGasLimit bfState == 0
        then pure unchanged
        else do
          ...
          if V.null newTrans
            then pure unchanged
              ...
              when (_bfGasLimit newState > _bfGasLimit bfState) $
                throwM $
                  MempoolFillFailure $
                    "Gas must not increase: " <> sshow (bfState, newState)
    ...
    splitResults (BlockFilling (BlockFill g rks i) success fails) (t, r) = case r of
      Right cr ->
        enforceUnique rks (requestKeyToTransactionHash $ P._crReqKey cr) >>= \rks' ->
          -- Decrement actual gas used from block limit
          return $
            BlockFilling
              (BlockFill (g - fromIntegral (P._crGas cr)) rks' i)
              (V.snoc success (t, cr))
              fails
      Left f ->
        enforceUnique rks (gasPurchaseFailureHash f) >>= \rks' ->
...
execLookupPactTxs restorePoint txs
  ...
  where
    go =
      getCheckpointer >>= \(!cp) -> case restorePoint of
        NoRewind _ ->
