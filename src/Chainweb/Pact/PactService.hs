...
import qualified Chainweb.Ranked as Ranked
...
execValidateBlock memPoolAccess hints forkInfo = do
    ...
    (rewoundTxs, validatedTxs, newConsensusState) <- withSavepoint sql ValidateBlockSavePoint $ do -- pactLabel "execValidateBlock" $ do
        let
            ...
            go acc (tip:chain) = do
                known <- Checkpointer.lookupBlockRanked (unwrapParent $ _evaluationCtxRankedParentHash tip)
                if known
                then return (Just (tip `NEL.cons` acc))
        ...
        latestBlockRewindable <- Checkpointer.lookupBlockRanked (_syncStateRankedBlockHash (_consensusStateLatest pactConsensusState))
        if atTarget
        then do
    ...
    getRewoundTxs rewindTargetHeight = do
        ...
        rewoundPayloads <- liftIO $ fmap fromJuste <$>
            lookupPayloadDataWithHeightBatch
                ...
                [(Just (Ranked.rank rbph), Ranked._ranked rbph) | rbph <- rewoundPayloadHashes]
        V.concat <$> traverse
