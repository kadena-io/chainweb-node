...
cancelSubmittedRequest statusRef = do
    maybeTid <- atomically $ do
        status <- readTVar statusRef
        case status of
            RequestFailed _ -> return Nothing
            RequestInProgress tid -> return (Just tid)
            RequestDone _ -> return Nothing
            RequestNotStarted -> Nothing <$ writeTVar statusRef (RequestFailed (toException AsyncCancelled))
    forM_ maybeTid $ \tid -> throwTo tid AsyncCancelled
