...
transactionFromText t =
  either (throwM . TextFormatException . sshow) return $
...
instance MerkleHashAlgorithm a => FromJSON (BlockPayload_ a) where
  parseJSON = withObject "BlockPayload" $ \o ->
    BlockPayload
      <$!> o .: "payloadHash"
      <*> o .: "transactionsHash"
      <*> o .: "outputsHash"
...
instance MerkleHashAlgorithm a => FromJSON (BlockTransactions_ a) where
  parseJSON = withObject "BlockTransactions" $ \o ->
    BlockTransactions
      <$!> o .: "transactionHash"
      <*> o .: "transaction"
      <*> o .: "minerData"
...
instance MerkleHashAlgorithm a => FromJSON (BlockOutputs_ a) where
  parseJSON = withObject "BlockOutputs" $ \o ->
    BlockOutputs
      <$!> o .: "outputsHash"
      <*> o .: "outputs"
      <*> o .: "coinbaseOutput"
...
instance MerkleHashAlgorithm a => FromJSON (TransactionTree_ a) where
  parseJSON = withObject "TransactionTree" $ \o ->
    TransactionTree
      <$!> o .: "hash"
...
merkleTreeFromJson = withText "MerkleTree" $ \t ->
...
verifyTransactionTree p =
  _transactionTreeHash p
...
instance MerkleHashAlgorithm a => FromJSON (OutputTree_ a) where
  parseJSON = withObject "OutputTree" $ \o ->
    OutputTree
      <$!> o .: "hash"
      <*> (o .: "tree" >>= merkleTreeFromJson)
...
blockPayload txs outs =
  fromLog $!
    newMerkleLog @a @ChainwebHashTag $
...
instance MerkleHashAlgorithm a => FromJSON (PayloadData_ a) where
  parseJSON = withObject "PayloadData" $ \o ->
    PayloadData
      <$!> o .: "transactions"
      <*> o .: "minerData"
      <*> o .: "payloadHash"
...
instance MerkleHashAlgorithm a => FromJSON (PayloadWithOutputs_ a) where
  parseJSON = withObject "PayloadWithOutputs" $ \o ->
    PayloadWithOutputs
      <$!> o .: "transactions"
      <*> o .: "minerData"
      <*> o .: "coinbase"
...
payloadWithOutputsToPayloadData o =
  PayloadData
