...
isMonotonicCutExtension c h = do
  ...
  where
    monotonic =
      _blockParent h == case c ^? ixg (_chainId h) . blockHash of
...
monotonicCutExtension c h =
  tryMonotonicCutExtension c h >>= \case
...
tryMonotonicCutExtension ::
  ...
  BlockHeader ->
  m (Maybe Cut)
tryMonotonicCutExtension c h =
  isMonotonicCutExtension c h >>= \case
...
applyJoin m =
  over unsafeCutHeaders projectChains
    <$> foldM
