...
import Chainweb.BlockCreationTime qualified as Chainweb
...
-- | Compute the Ethereum Timestamp for a block headder
--
-- In chainweb the payload time is the block creation of the parent header. It
-- must be strictly monotonically increasing at microsecond precision.
--
-- The EVM requires that timestamps of EVM payload headers are stricly
-- monotonically increasing at second precision with respect to the parent
-- relation.
--
-- Chainweb also requires that all data in the preimage of the block payload
-- hash is derived deterministcally from the current payload provider state (as
-- captured by preimage of the previous block payload hash), the user
-- transactions, and the chainweb parent header (the ParentBeaconBlockRoot).
--
-- In chainweb it can happen that blocks are less than 1s appart. The
-- constraints for timestamps are satisfied by using the smallest integral value
-- that is larger or equal than the block creation time of the chainweb parent
-- timestamp (the ParentBeaconBlockRoot) and strictly larger than the timestamp
-- of the EVM parent header.
--
timestamp
    :: Timestamp
        -- ^ Timestamp of the EVM parent header
    -> Chainweb.BlockCreationTime
        -- ^ BlockCreationTime of the ParentBeaconBlockRoot
    -> Timestamp
timestamp (Timestamp e) (Chainweb.BlockCreationTime (Time (TimeSpan (Micros t)))) =
    Timestamp $ max (e + 1) cwTimestamp
  where
    cwTimestamp = ceiling (t % 1_000_000)
...
beaconBlockRootToChainwebBlockHash :: ParentBeaconBlockRoot -> Chainweb.BlockHash
beaconBlockRootToChainwebBlockHash bs =
    case runGetS @_ @Chainweb.BlockHash Chainweb.decodeBlockHash (bytes bs) of
        Left e -> error $ "Chainweb.PayloadProvider.EVM.Header.beaconBlockRootToChainwebBlockHash. invalid beacon block root: " <> sshow e
        Right x -> x
