...
pMinimalProviderConfig = id
  ...
  where
    ...
    pChainId = unsafeChainId <$> option auto
        % long "default-redeem-chain"
        <> help "chain on which block rewards from the default payload provider can be claimed"

    pAccount :: OptionParser Account
    pAccount = option textReader
        % long "default-redeem-account"
...
data MinimalPayloadProvider = MinimalPayloadProvider
    ...
    , _minimalCandidatePayloads :: !(MapTable RankedBlockPayloadHash Payload)
...
newMinimalPayloadProvider logger v c rdb mgr conf
    ...
    | otherwise = do
        ...
        return MinimalPayloadProvider
            ...
            , _minimalLogger = logFunction logger
            }
  where
...
minimalSyncToBlock p h i = do
    ...
    pruneCandidates p (_forkInfoTargetState i)
