...
-- |
--
-- * For Pact db is (PayloadDb tbl)
-- * For EVM db is (HeaderDb tbl)
--
-- NOTE:
-- It seems that currently the the type indexes are only used for dispatching
-- the server implementation.
--
-- However, going forward we may also use those for dispatching the actual db
-- implementation.
--

newtype PayloadDb' db (v :: ChainwebVersionT) (c :: ChainIdT) = PayloadDb' db

data SomePayloadDb db = forall v c
    . (KnownChainwebVersionSymbol v, KnownChainIdSymbol c)
    => SomePayloadDb (PayloadDb' db v c)

somePayloadDb :: forall db . ChainwebVersion -> ChainId -> db -> SomePayloadDb db
somePayloadDb v cid db = runIdentity $ do
    ...
    return $! SomePayloadDb (PayloadDb' @db @vt @cidt db)
...
--
newtype BatchBody = BatchBody [(BlockHeight, BlockPayloadHash)]

instance FromJSON BatchBody where
    -- parse the WithHeight variant as an Object like so:
    --    { "heights": [ height1, height2, ... ], "hashes": [ "hash1", "hash2", ... ] }
    parseJSON = withObject "BatchBody" $ \o ->
        BatchBody <$> (zip <$> o .: "heights" <*> o .: "hashes")

instance ToJSON BatchBody where
    toJSON (BatchBody xs) = object
        [ "heights" .= map fst xs
        , "hashes" .= map snd xs
        ]
