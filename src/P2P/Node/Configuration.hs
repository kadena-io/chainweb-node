...
validateP2pConfiguration c = do
  ...
  when (_p2pConfigIgnoreBootstrapNodes c && null (_p2pConfigKnownPeers c)) $
    tell $
  ...
  when (_p2pConfigPrivate c && null (_p2pConfigKnownPeers c)) $
    tell $
  ...
  when (_p2pConfigSessionTimeout c < 120) $
    tell $
  ...
  when (_p2pConfigMaxSessionCount c < 3) $
    tell $
  ...
  when (_p2pConfigMaxSessionCount c > 30) $
    throwError
  ...
  when (_p2pConfigBootstrapReachability c > 1) $
    throwError
...
instance FromJSON (P2pConfiguration -> P2pConfiguration) where
  parseJSON = withObject "P2pConfiguration" $ \o ->
    id
...
instance FromJSON P2pConfiguration where
  parseJSON = withObject "P2pExampleConfig" $ \o ->
    P2pConfiguration
      <$> o .: "peer"
      <*> o .: "maxSessionCount"
      <*> o .: "maxPeerCount"
