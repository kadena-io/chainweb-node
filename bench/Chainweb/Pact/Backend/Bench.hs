...
-- also present in Chainweb.Test.Pact.Checkpointer
cpRewindAndExtend
  :: (Monoid q)
  => Checkpointer logger
  -> ParentContext
...
cpRewindAndExtend cp pc blks = snd <$> _cpRewindAndExtend cp pc
  ...
  (\dbenv _pc i -> fmap (,fst (blks !! i)) $ snd (blks !! i) (_cpPactDbEnv dbenv))
...
cpBenchNoRewindOverBlock transactionCount cp = C.env setup' $ \ ~(ut) ->
  ...
  where
    ...
    pc01 = childOf genesisPc hash01
    setup' = do
        [usertablename] <- cpRewindAndExtend cp genesisPc
    ...
    go mblock (NoopNFData ut) = do
        ...
        let pc' = childOf pc hash'
        modifyMVar_ mblock
            $ \_ -> do
              return (bytestring', pc')
        void $ cpRewindAndExtend cp pc
...
cpBenchOverBlock transactionCount cp = C.env setup' $ \ ~(ut) ->
  ...
  where
    ...
    setup' = do
        [usertablename] <- cpRewindAndExtend cp genesisPc [(pc01, \db ->
...
cpBenchLookupProcessedTx transactionCount cp = C.env setup' $ \ ~(ut) ->
  ...
  where
    ...
    go (NoopNFData _) = do
        _cpReadFrom (_cpReadCp cp) pc02 $ \dbEnv ->
