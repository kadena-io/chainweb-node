...
module Chainweb.Pact.Backend.Bench (bench) where
...
bench :: C.Benchmark
bench =
  C.bgroup "pact-backend" $
    play
      [ pactSqliteWithBench False . benchUserTable,
...
cpBenchNoRewindOverBlock transactionCount cp = C.env (setup' cp) $ \ ~(ut) ->
  ...
  where
    name =
      "noRewind/transactionCount="
        ++ show transactionCount
    setup' CheckpointEnv {..} = do
      usertablename <-
        _cpRestore _cpeCheckpointer Nothing >>= \case
          PactDbEnv' db ->
    ...
    go CheckpointEnv {..} mblock (NoopNFData ut) = do
      ...
      void $
        _cpRestore _cpeCheckpointer (Just (blockheight, hash)) >>= \case
          PactDbEnv' pactdbenv ->
...
writeRow (PactDbEnv pdb e) writeType ut f k i =
  _writeRow
    ...
    ut
    k
    (RowData RDV1 (ObjectMap $ M.fromList [(f, (RDLiteral (LInteger i)))]))
    e
