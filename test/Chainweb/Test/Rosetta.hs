...
import Pact.Types.Runtime (RowKey (..), TxId (..))
import Rosetta
import Test.Tasty
...
tests =
  testGroup
    "Chainweb.Test.Rosetta.UnitTests"
  ...
  where
    case1 =
      let noPrevLogs = mockPrevTxs []
          unique =
            [ cases 0 [createCase "k1" 1.0 (bd 1.0)],
    ...
    case2 =
      let allHavePrevLogs =
          ...
          unique =
            [ cases 0 [createCase "k1" 1.0 (bd 0.0)],
    ...
    case3 =
      let onlyOneSeenPrevLogs = mockPrevTxs [("k1", 1.0)]
          unique =
            [ cases 0 [createCase "k1" 0.5 (bd $ negate 0.5)],
    ...
    case5 =
      let prevLogs = mockPrevTxs [("k1", 10.0)]
          repeated =
            [ cases
                0
                [ createCase "k1" 9.99 (bd $ negate 0.01),
                  createCase "k1" 9.0 (bd $ negate 0.99),
                  createCase "k1" 9.5 (bd 0.5)
                ],
              cases
                1
                [ createCase "k1" 1.0 (bd $ negate 8.5),
    ...
    case6 =
      let prevLogs = mockPrevTxs [("miner", 10.0), ("sender1", 10.0), ("sender2", 10.0)]
          mock =
            [ cases 0 [createCase "miner" 12.0 (bd 2.0)],
              --------------------------------------------------------
          ...
          { _matchRosettaTx_caseLabel = "Coin Tx, Successful",
            ...
            _matchRosettaTx_operations =
              [ mops (TxId 7) [mop FundTx (opId 0) []],
                mops
                  (TxId 8)
                  [ mop TransferOrCreateAcct (opId 1) [],
                    mop TransferOrCreateAcct (opId 2) [opId 1]
                  ],
                mops
                  (TxId 9)
                  [ mop GasPayment (opId 3) [opId 0],
...
matchNonGenesisSingleTransactionsToLogs = do
  ...
  where
    ...
    run trk = getActual cases f
      where
        f logs cid initial rest = nonGenesisTransaction logs cid initial rest (textToRk trk)
          ...
          assertFailure $
            adjust msg $
              adjust "request key missing. Expected request key " $
                show $
                  _transactionId_hash $
                    _transaction_transactionId expect
...
checkKDAToRosettaAmount = do
  assertEqual
    "normal: 123.0"
    (rosettaAmount normalStandard)
    normalAtomic
  assertEqual
    "min precision: 0.123456789123"
    (rosettaAmount smallStandard)
    smallAtomic
  assertEqual
    "big with min precision: 123456789123.123456789123"
    (rosettaAmount bigStandard)
    bigAtomic
  assertEqual
    "smaller than min precision: 0.123456789123456789"
    (rosettaAmount reallySmallStandard)
    reallySmallAtomic
  assertEqual
    "really big with min precision: 123456789123456789.123456789123"
    (rosettaAmount reallyBigStandard)
    reallyBigAtomic
  where
...
checkValidateNetwork = do
  assertEqual
    ...
    (Right "0")
  assertEqual
    "invalid blockchain name"
    (run invalidBlockchainName)
    (Left RosettaInvalidBlockchainName)
  assertEqual
    "mismatched network name"
    (run mismatchedNetName)
    (Left RosettaMismatchNetworkName)
  assertEqual
    "chain id unspecified"
    (run chainIdUnspecified)
    (Left RosettaChainUnspecified)
  assertEqual
    "invalid chain id"
    (run invalidChainId)
