...
runQuery ::
  ...
  IO a
runQuery mgr v q =
  HTTP.runClientM q (endpoint mgr v) >>= \case
    Left e -> error (show e)
    Right x -> return x
...
getHeaderByHeight mgr v cid height = do
  ...
  when (curHeight < height) $
    error $
      "getHeaderByHeight: height " <> sshow height <> " is in the future. Current height " <> sshow curHeight
  page <- runQuery mgr v $ query curHash
  when (_pageLimit page < 1) $
    error $
      "getHeaderByHeight: no header found for height " <> sshow height <> " on chain " <> sshow cid
  when (_pageLimit page > 1) $
    error $
      "getHeaderByHeight: expect just one result but got " <> sshow (_pageLimit page)
