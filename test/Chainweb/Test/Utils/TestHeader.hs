...
instance FromJSON TestHeader where
  parseJSON = withObject "TestHeader" $ \o ->
    TestHeader
      <$> o .: "header"
...
genesisTestHeader ::
  ...
  c ->
  TestHeader
genesisTestHeader v cid =
  TestHeader
    { _testHeaderHdr = gen,
      _testHeaderParent = ParentHeader gen,
      _testHeaderAdjs =
        ParentHeader . genesisBlockHeader (_chainwebVersion v)
...
queryTestHeader v c h = do
  ...
  ads <-
    itraverse (\ac a -> ParentHeader <$> getHeaderByHash mgr ver ac a) $
      _getBlockHashRecord $
