...
extendAwait ::
  ...
  (Cut -> Bool) ->
  IO (Maybe Cut)
extendAwait cdb pact i p =
  race gen (awaitCut cdb p) >>= \case
...
randomBlockHeader cutDb = do
  curCut <- _cut cutDb
  allBlockHeaders <- webEntries (view cutDbWebBlockHeaderDb cutDb) $ \s ->
    s
