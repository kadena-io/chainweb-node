...
simpleSessionTests rdb tls version =
  withBlockHeaderDbsResource rdb version $ \dbs ->
    withBlockHeaderDbsServer tls version dbs (return noMempool) $
      \env ->
        testGroup "client session tests" $
...
httpHeaderTests envIO cid =
  testGroup
    ("http header tests for chain " <> sshow cid)
  ...
  where
    go name run = testCase name $ do
      ...
      res <-
        flip runClientM_ env $
          modifyResponse checkHeader $
...
simpleClientSession envIO cid =
  ...
  where
    session version dbs step = do
      ...
      assertExpectation
        "header client returned wrong entry"
        (Expected gbh0)
        (Actual gen0)
      ...
      assertExpectation
        "headersClient returned wrong number of entries"
        (Expected 4)
      ...
      assertExpectation
        "hashesClient returned wrong number of entries"
        (Expected $ _pageLimit bhs2)
        (Actual $ _pageLimit hs2)
      assertExpectation
        "hashesClient returned wrong hashes"
      ...
      bhs3 <-
        branchHeadersClient
          ...
          (BranchBounds mempty mempty)
      assertExpectation
        "branchHeadersClient returned wrong number of entries"
        (Expected 0)
      ...
      hs3 <-
        branchHashesClient
          ...
          (BranchBounds mempty mempty)
      assertExpectation
        "branchHashesClient returned wrong number of entries"
        (Expected 0)
...
pagingTest ::
  ...
  IO TestClientEnv_ ->
  TestTree
pagingTest name getDbItems getKey fin request envIO =
  testGroup
    name
  ...
  where
    session step ents cid n next = do
      ...
      assertExpectation
        "result has wrong page 'limit' value"
        (Expected . maybe id min n . int $ length ents)
        (Actual $ _pageLimit r)
      assertExpectation
        "result contains wrong page 'items'"
        (Expected . maybe id (take . int) n $ ents)
        (Actual $ _pageItems r)
      assertExpectation
        "result contains wrong page 'next' value"
