...
treeDbInvariants ::
  ...
  RunStyle ->
  TestTree
treeDbInvariants f rs =
  testGroup
    "TreeDb Invariants"
    [ testGroup "Properties" $
        schedule
          rs
          [ testGroupSch "TreeDb Shape" $
              schedule
                rs
            ...
            testGroupSch "Basic Streaming" $
              schedule
                rs
                [ testGroupSch "Self-reported Stream Length" $
                    schedule
                      rs
...
entryOrder_prop f (SparseTree t0) = ioProperty . withTreeDb f t $ \db _ -> do
  ...
  where
    g acc h =
      let acc' = S.insert (_blockHash h) acc
...
prop_seekLimitStream_limit l i =
  i <= len l ==>
    actual === expected
