...
tests db h0 =
  testGroupSch
    "mempool-consensus-quickcheck-tests"
...
prop_validTxSource db genBlock = monadicIO $ do
  ...
  assert $
    (reIntroTrans `isSubsetOf` fiOldForkTrans)
...
instance Show ForkInfo where
  show ForkInfo {..} =
    "ForkInfo - forkHeight: "
      ++ show fiForkHeight
      ++ ", leftBranchHeight: "
      ++ show fiLeftBranchHeight
      ++ ", rightBranchHeight: "
      ++ show fiRightBranchHeight
      ++ "\n\t"
      ++ ", number of old forkTrans: "
      ++ show (HS.size fiOldForkTrans)
      ++ debugTrans "oldForkTrans" fiOldForkTrans
      ++ ", number of new forkTrans: "
      ++ show (HS.size fiNewForkTrans)
      ++ debugTrans "newForkTrans" fiNewForkTrans
      ++ "\n\t"
      ++ "'head' of old fork:"
      ++ "\n\t\tblock height: "
      ++ show (_blockHeight fiOldHeader)
      ++ "\n\t\tblock hash: "
      ++ show (_blockHash fiOldHeader)
      ++ "\n\t"
      ++ "'head' of new fork:"
      ++ "\n\t\tblock height: "
      ++ show (_blockHeight fiNewHeader)
      ++ "\n\t\tblock hash: "
      ++ show (_blockHash fiNewHeader)
