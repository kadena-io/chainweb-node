...
getHistoricalLookupNoTxs key assertF refIO reqIO = testCase msg $ do
  ...
  where
    msg = T.unpack $ "getHistoricalLookupNoTxs: " <> key
...
getHistoricalLookupWithTxs key assertF refIO reqIO = testCase msg $ do
  ...
  histLookup q h key >>= assertF
  where
    msg = T.unpack $ "getHistoricalLookupWithTxs: " <> key
            ...
            block =
              Transactions
                ...
                ( CommandResult
                    (RequestKey (Hash "h"))
                    Nothing
                    (PactResult $ Right $ pString "output")
                    0
                    Nothing
                    Nothing
                    Nothing
...
mempoolCreationTimeTest mpRefIO reqIO = testCase "mempoolCreationTimeTest" $ do
  ...
  void $
    forSuccess "mempoolCreationTimeTest: pre-insert tx" $
...
goldenNewBlock name mp mpRefIO reqIO = golden name $ do
  ...
  resp <-
    forSuccess ("goldenNewBlock:" ++ name) $
