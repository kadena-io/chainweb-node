...
runBlockE q bdb timeOffset = do
  ...
  !bip <- fromJuste <$> newBlock noMiner True (ParentHeader ph) q
...
newBlockAndContinue refIO reqIO = testCase "newBlockAndContinue" $ do
  ...
  !bipStart <- fromJuste <$> newBlock noMiner True (ParentHeader genesisHeader) q
  let ParentHeader ph = _blockInProgressParentHeader bipStart
  Just bipContinued <- continueBlock bipStart q
  Just bipFinal <- continueBlock bipContinued q
  ...
  !bipAllAtOnce <- fromJuste <$> newBlock noMiner True (ParentHeader genesisHeader) q
...
newBlockNoFill refIO reqIO = testCase "newBlockNoFill" $ do
  ...
  !noFillPwo <- blockInProgressToPayloadWithOutputs . fromJuste <$>
  ...
  !fillPwo <- blockInProgressToPayloadWithOutputs . fromJuste <$>
...
newBlockAndValidationFailure refIO reqIO = testCase "newBlockAndValidationFailure" $ do
  ...
  !bip <- fromJuste <$> newBlock noMiner True (ParentHeader genesisHeader) q
  let nb = blockInProgressToPayloadWithOutputs bip
...
badlistNewBlockTest mpRefIO reqIO = testCase "badlistNewBlockTest" $ do
  ...
  !bip <- fromJuste <$> newBlock noMiner True (ParentHeader genesisHeader) reqQ
...
goldenNewBlock name mpIO mpRefIO reqIO = golden name $ do
    ...
    !blockInProgress <- fromJuste <$> newBlock noMiner True (ParentHeader genesisHeader) reqQ
