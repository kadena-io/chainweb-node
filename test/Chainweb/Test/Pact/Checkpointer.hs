...
tests = testGroupSch "Checkpointer"
  [ testCase "testInMemory" testInMemory, testCase "testRelationalKeyset" testKeyset, testCase "testRelational" testRelational ]
...
runOnVersionTables f g h = do
  ...
  vtables <-  callDb $ \db -> qry_ db "SELECT * FROM VersionedTables;" [RText, RInt, RInt]
  void $ liftIO $ f blocktable
  void $ liftIO $ g versionhistory
  liftIO $ h vtables

keysetTest :: CheckpointEnvNew SQLiteEnv -> Assertion
...
relationalTest CheckpointEnvNew {..} = do
  ...
  withBlockEnv blockenv02 $ \benv -> do
    ...
    void $ runExec chainwebpactdb benv (Just $ ksData "2") $ defModule "2"

    runExec chainwebpactdb benv Nothing "(m2.readTbl)"
      >>= \EvalResult{..} -> Right _erOutput @?= traverse toPactValue [tIntList [1]]
  ...
  withBlockEnv blockenv03 $ \benv -> do
    ...
    void $ runExec chainwebpactdb benv Nothing "(m2.insertTbl 'b 2)"

    runExec chainwebpactdb benv Nothing "(m2.readTbl)"
      >>= \EvalResult{..} -> Right _erOutput @?= traverse toPactValue [tIntList [1,2]]


  discardNew _cpeCheckpointerNew >>= assertEitherSuccess "discard for block 02"
...
------------------------------------------------------------------
  ...
  withBlockEnv blockenv02Fork $ \benv -> do
    ...
    runExec chainwebpactdb benv Nothing "(m2.readTbl)" >>= \EvalResult{..} -> Right _erOutput @?= traverse toPactValue [tIntList [1]]
