...
runExec cp (pactdbenv, coredb) eData eCode = do
  ...
  where
    ...
    cmdenv = TransactionEnv Transactional (Left pactdbenv) (_cpLogger $ _cpReadCp cp) Nothing def
             noSPVSupport Nothing 0.0 (RequestKey h') 0 def Nothing
             noSPVSupport Nothing 0.0 (RequestKey h') 0 def Nothing Nothing
    cmdst = TransactionState mempty mempty 0 Nothing (Left $ _geGasModel freeGasEnv) mempty
...
cpReadFrom cp pc f = do
  _cpReadFrom
    (_cpReadCp cp)
    (ParentHeader <$> pc)
    (\env -> f (_cpPactDbEnv env, _cpPactCoreDbEnv env)) >>= \case
