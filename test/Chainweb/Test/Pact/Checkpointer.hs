...
testModuleName = withResourceT withTempSQLiteResource $
    runSQLite' $ \resIO -> testCase "testModuleName" $ do
        ...
        let
          ...
          pc00 = (genesisParentContext testVer testChainId)
            { _parentContextHash = hash00, _parentContextHeight = 0 }
        cpRewindAndExtend cp Nothing
          [(pc00, \_ -> return ())]

        let
          hash01 = getArbitrary 1
          pc01 = childOf pc00 hash01
        cpRewindAndExtend cp (Just pc00)
          [(pc01, \_ -> return ())
          ]
        let
          hash02 = getArbitrary 2
          pc02 = childOf pc01 hash02
        cpRewindAndExtend cp (Just pc01)
...
keysetTest c = testCaseSteps "Keyset test" $ \next -> do
    ...
    next "init"
    cpRewindAndExtend cp Nothing [(pc00, \_ -> pure ())]
    ...
    cpRewindAndExtend cp (Just pc00) [(pc11, \dbEnv ->
...
checkpointerTest name relational cenvIO = testCaseSteps name $ \next -> do
    ...
    void $ readFrom (Just genesisPc) $ \dbEnv -> do
    -- start a pact
    ...
    let pc01 = childOf genesisPc hash01
    rewindAndExtend (Just genesisPc)
      [(pc01, \dbEnv -> do
        void $ runExec cenv dbEnv Nothing "(m1.insertTbl 'b 2)"
        runExec cenv dbEnv Nothing "(m1.readTbl)"
    ...
    void $ expectException $
      readFrom (Just pc10 { _parentContextHeight = 13 }) $ \_ -> return ()
...
cpRewindAndExtend cp pc blks = snd <$> _cpRewindAndExtend cp pc
  ...
  (\dbenv _pc i -> snd (blks !! i) (_cpPactDbEnv dbenv))

childOf :: ParentContext -> BlockHash -> ParentContext
childOf pc bh = pc & parentContextHeight +~ 1 & parentContextHash .~ bh
