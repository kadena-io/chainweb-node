...
chainweb213Test = do
  ...
  where
    ...
    buildModCmd1 =
      buildBasic $
        mkExec' $
          mconcat ["(namespace 'free)", "(module mtest G (defcap G () true) (defun a () true))"]
    buildModCmd2 =
      buildBasicGas 70000 $
        mkExec' $
          mconcat ["(namespace 'free)", "(module mtest2 G (defcap G () true) (defun a () false))"]
    buildDbMod =
      buildBasicGas 70000 $
        mkExec' $
          mconcat
...
pact45UpgradeTest = do
  ...
  runBlockTest
    [ PactTxTest
        ...
        $ assertTxSuccess "mod53 table created"
        $ pString "TableCreated"
    ]
  runBlockTest
    [ PactTxTest (buildSimpleCmd "(enforce false 'hi)") $
      ...
      PactTxTest
        (buildBasicGas 70000 $ tblModule "Tbl")
        $ assertTxSuccess "mod53 table update succeeds"
        $ pString "TableCreated",
  ...
  runBlockTest
    [ PactTxTest describeModule $
      ...
      PactTxTest isPrincipal $
        assertTxFailure
          "Should not resolve new pact native: is-principal"
          "Cannot resolve is-principal",
      PactTxTest typeOfPrincipal $
        assertTxFailure
          "Should not resolve new pact native: typeof-principal"
...
pact420UpgradeTest = do
  ...
  runBlockTest
    [ PactTxTest buildNewNatives420FoldDbCmd
        ...
        $ pList
          [ pObject [("a", pInteger 1), ("b", pInteger 1)],
  ...
  where
    buildFdbCmd =
      buildBasic $
        mkExec' $
          mconcat ["(namespace 'free)", moduleDeclaration, inserts]
      where
        moduleDeclaration =
  ...
  runBlockTest
    [ PactTxTest pointAddTx $
        assertTxSuccess
          ...
          ( pObject
              [ ("x", pInteger 1368015179489954701390400359078579693043519447331113978918064868415326638035),
...
pact4coin3UpgradeTest = do
        ...
        [ PactTxTest buildHashCmd $ \cr -> do
          ...
          PactTxTest buildReleaseCommand $ \cr -> do
            ...
            allocEv <-
              mkEvent
                "RELEASE_ALLOCATION"
                [pString "allocation00", pDecimal 1000000.0]
                "coin"
                v3Hash
  ...
  where
    ...
    badKeyset =
      buildBasic $
        mkExec "(read-keyset 'ks)" $
          object ["ks" .= ["badkey" :: T.Text]]

    buildNewNatives40Cmd =
      buildBasic $
...
buildXProof scid bh i sendTx = do
  ...
  proof <-
    liftIO $
      ContProof . B64U.encode . encodeToByteString
...
getPactId =
  fromMaybeM (userError "no continuation")
    . preview (crContinuation . _Just . pePactId)

buildXReceive ::
  (ContProof, PactId) ->
  MempoolCmdBuilder
buildXReceive (proof, pid) =
  buildBasic $
...
txResult i =
  txResults >>= \rs -> case preview (ix i) rs of
    Nothing ->
