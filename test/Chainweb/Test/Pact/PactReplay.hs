...
import Chainweb.BlockHeaderDB.Internal (RankedBlockHeader (..), unsafeInsertBlockHeaderDb, _chainDbCas)
...
import Chainweb.Utils (T3 (..), catchAllSynchronous, sshow, tryAllSynchronous)
       ...
       in testGroup
            ...
            [ withPactTestBlockDb
              ...
              after AllSucceed "reject-dupes" $
                let deepForkLimit = 4
                 in withPactTestBlockDb
                      testVer
                      cid
                      Quiet
                      rdb
                      mp
                      (forkLimit deepForkLimit)
...
testDupes mpio genesisBlock iop = do
  ...
  expectException newblock payload $
    liftIO $
    ...
    expectException newblock payload act = do
      ...
      where
        wrap = do
          ...
          let msg =
                concat
                  [ "expected exception on dupe block. new block header:\n",
                    sshow newblock2,
                    "\nnew payload: \n",
                    sshow payload2,
                    "\nprev block: \n",
...
mineBlock ph nonce iop =
  timeout 5000000 go >>= \case
