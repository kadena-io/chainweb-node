...
execTest runPact request = _trEval request $ do
    ...
    results <- runPact $ execTransactions (Just nullBlockHash) defaultMiner trans (EnforceCoinbaseFailure True) (CoinbaseUsePrecompiled True)
...
execTxsTest runPact name (trans',check) = testCaseSch name (go >>= check)
  where
    go = do
      trans <- trans'
      results' <- try $ runPact $ execTransactions (Just nullBlockHash) defaultMiner trans (EnforceCoinbaseFailure True) (CoinbaseUsePrecompiled True)
      case results' of
