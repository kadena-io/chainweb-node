            ...
            testGroup
              ...
              [ testCaseSteps "await network" $ \step ->
                ...
                after AllSucceed "await network" $
                  withRequestKeys iot iomvar net $
                    responseGolden net,
...
responseGolden networkIO rksIO = golden "remote-golden" $ do
  ...
  let values =
        mapMaybe
          (\rk -> _crResult <$> HashMap.lookup rk theMap)
...
localPreflightSimTest iot nio = testCaseSteps "local preflight sim test" $ \step -> do
  ...
  sigs1' <-
    testKeyPairs sender00 Nothing >>= \case
  ...
  runLocalPreflightClient sid cenv cmd7 >>= \case
    ...
    Right (LocalResultWithWarns _ ws) -> case ws of
      [w]
        | "decimal/integer operator overload" `T.isInfixOf` w ->
            pure ()
...
expectSendFailure ::
  ...
  IO (Either b a) ->
  Assertion
expectSendFailure expectErr act =
  tryAllSynchronous act >>= \case
...
ethSpvTest iot nio = testCaseSteps "eth spv client tests" $ \step -> do
  req <-
    A.eitherDecodeFileStrict' "test/pact/eth-spv-request.json" >>= \case
...
txTooBigGasTest iot nio = testCaseSteps "transaction size gas tests" $ \step -> do
  ...
  assertEqual
    "LOCAL: expect gas error after discounting initial gas charge"
    gasError1
    (getFailureMsg $ resultOf res1Local)

  case res1Send of
    Left e -> assertFailure $ "test failure for discounting initial gas charge: " <> show e
    Right cr ->
      assertEqual
        "SEND: expect gas error after discounting initial gas charge"
        (Just gasError1)
        (getFailureMsg . resultOf <$> cr)
  where
    sid = unsafeChainId 0
    gasError0 =
      Just $
...
awaitCutHeight step cenv i = do
  result <-
    retrying testRetryPolicy checkRetry $
      const $
        runClientM (cutGetClient v) cenv
  case result of
    Left e -> throwM e
    Right x
      ...
      | otherwise ->
          throwM $
            SlowChain $
              "retries exhausted: waiting for cut height "
                <> sshow i
                <> " but only got "
                <> sshow (_cutHashesHeight x)
  where
    checkRetry s (Left e) = do
      step $
        "awaiting cut of height "
          <> show i
          <> ". No result from node: "
          <> show e
          <> " ["
      ...
      | otherwise = do
          step $
            "awaiting cut of height "
              <> show i
              <> ". Current cut height: "
              <> show (_cutHashesHeight c)
              <> ". Current block heights: "
              <> show (_bhwhHeight <$> _cutHashes c)
              <> " ["
              <> show (view rsIterNumberL s)
              <> "]"
          return True
...
pactDeadBeef =
  let (TransactionHash b) = deadbeef
