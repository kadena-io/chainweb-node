...
import Control.Lens (makeLenses, view, _3)
...
mkXChainTransferCap sender receiver amount cid =
  mkCoinCap
    "TRANSFER_XCHAIN"
    [ pString sender,
      pString receiver,
...
buildCwCmd cmd =
  buildRawCmd cmd >>= \c -> case verifyCommand c of
...
setOneShotMempool mpRefIO mp = do
  ...
  setMempool mpRefIO $
    mp
      { mpaGetBlock = \g v i a e ->
          readIORef oneShot >>= \case
...
withPactTestBlockDb version cid logLevel rdb mempoolIO pactConfig f =
  ...
  where
    startPact bdbio iodir = do
      ...
      a <-
        async $
          runForever (\_ _ -> return ()) "Chainweb.Test.Pact.Utils.withPactTestBlockDb" $
