...
checkResult co ci expect =
  assertSatisfies
    ("result on chain " ++ show ci ++ " contains '" ++ show expect ++ "'")
    (HM.lookup (unsafeChainId ci) co)
    (isInfixOf expect . show)
      ...
      | otherwise =
          readIORef ref >>= \case
            ...
            False -> do
              ...
              let q
                    | bridge = object [("proof", String $ encodeB64UrlNoPaddingText $ encodeToByteString pf)]
                    | otherwise = toJSON pf
              cmd <-
                buildCwCmd $
                  set cbSigners [mkSigner' sender00 []] $
