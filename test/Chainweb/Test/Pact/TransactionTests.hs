...
baseInjTest =
  mkCoinbaseCmd badMinerId minerKeys0 (ParsedDecimal 1.0) >>= \case
...
fixedInjTest = case exec of
  ExecMsg (ParsedCode pccode _pcexps) _pmdata
    | isInfixOf "coinbase" pccode ->
        assertFailure $
          "Precompiled statement contains exploitable code: "
            <> unpack pccode
    | isInfixOf "read-keyset" pccode ->
        assertFailure $
          "Precompiled statement contains exploitable code: "
            <> unpack pccode
    | otherwise -> return ()
...
testCoinbase797DateFix = testCaseSteps "testCoinbase791Fix" $ \step -> do
  ...
  where
    doCoinbaseExploit pdb mc height localCmd precompile testResult = do
      ...
      void $
        applyCoinbase
          ...
          (CoinbaseUsePrecompiled precompile)
          mc
      ...
      CommandResult _ _ (PactResult pr) _ _ _ _ _ <-
        evalTransactionM tenv txst $!
    ...
    mkTestParentHeader h =
      ParentHeader $
        (someBlockHeader (FastTimedCPM singleton) 10)
          { _blockHeight = h
          }
...
testCoinbaseEnforceFailure = do
  ...
  case r of
    Left e ->
      if isInfixOf "CoinbaseFailure" (sshow e)
        then return ()
        ...
        assertFailure $
          intercalate "\n" $
            [ "matchLogs: length mismatch "
                <> show (length actualResults)
                <> " /= "
                <> show (length expectedResults),
