...
import Chainweb.Mempool.Mempool (BlockFill (..), MempoolBackend (..), TransactionHash (..), mockBlockGasLimit)
import Chainweb.MerkleUniverse
import Chainweb.Miner.Config
import Chainweb.Miner.Pact
import Chainweb.Pact.Backend.Types (SQLiteEnv (..))
import Chainweb.Pact.Backend.Utils (chainwebPragmas, closeSQLiteConnection, openSQLiteConnection)
...
import Control.Monad.Catch (MonadThrow, bracket, finally)
...
import Test.Tasty.QuickCheck (discard, property, testProperty, (.&&.))
...
withRocksResource m = withResource create destroy wrap
  where
    ...
    destroy (dir, rocks, opts) =
      closeRocksDb rocks
        `finally` R.freeOpts opts
        `finally` destroyRocksDb dir
...
assertExpectation ::
  ...
  Actual a ->
  m ()
assertExpectation msg expected actual =
  liftIO $
    assertBool
...
assertLe msg actual expected =
  ...
  where
    msg_ =
      T.unpack msg
        <> ", expected: <= "
        <> show (getExpected expected)
        <> ", actual: "
        <> show (getActual actual)
...
assertInfix msg contents value =
  assertSatisfies
...
withNodes_ ::
  ...
  TestTree
withNodes_ logger v testLabel rdb n f =
  withResource
    start
    (cancel . fst)
...
config ::
  ...
  ChainwebConfiguration
config ver n =
  defaultChainwebConfiguration ver
...
bootstrapConfig conf =
  ...
  where
    peerConfig =
      head (bootstrapPeerConfig $ _configChainwebVersion conf)
