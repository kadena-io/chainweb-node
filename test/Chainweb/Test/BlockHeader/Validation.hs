    ...
    f now (i, (h, expectedErrs)) =
      try (validateBlockHeaderM now (testHeaderChainLookup h) (_testHeaderHdr h)) >>= \case
        ...
        Left ValidationFailure {_validationFailureFailures = errs}
          | sort errs /= sort expectedErrs ->
              assertFailure $
                "Validation of test case "
                  <> sshow i
                  <> " failed with unexpected errors for BlockHeader"
                  <> ", expected: "
                  <> sshow expectedErrs
                  <> ", actual: "
                  <> sshow errs
                  <> ", header: "
                  <> sshow (_blockHash $ _testHeaderHdr h)
                  <> ", height: "
                  <> sshow (_blockHeight $ _testHeaderHdr h)
          | otherwise -> return ()
...
validationFailures =
  [ ( hdr & testHeaderHdr . blockCreationTime .~ BlockCreationTime (Time maxBound),
    ...
    ( hdr
        & testHeaderHdr . blockFlags .~ fromJuste (runGetS decodeFeatureFlags badFlags)
...
daValidation =
  ...
  where
    ...
    -- From mainnet
    hdr =
      set (h . blockChainwebVersion) Development $
