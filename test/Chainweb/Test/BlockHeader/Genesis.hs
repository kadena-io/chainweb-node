...
tests = testGroup "Chainweb.Test.BlockHeader.Genesis"
    [ testGroup "genesis header golden tests" $ versionBlockHashTree <$>
        [ RecapDevelopment
        , Testnet04
        , Mainnet01
        ]
...
versionBlockHashTree :: ChainwebVersion -> TestTree
versionBlockHashTree v = golden (sshow v <> "-block-hashes") $
...
graphTransitionTargetTests = testGroup "graph transition genesis targets"
    ...
    [ testProperty "mainnet20InitialHashTarget deserialization" $
        Just (Mainnet01 ^?! versionGenesis . genesisBlockTarget . onChain (unsafeChainId 10)) === (HashTarget . (4 *) <$> decodePowHashNat64 "DOordl9cgfs4ZTBdFnbjRW5th-hW-pL33DIAAAAAAAA")
    , testProperty "mainnet20InitialHashTarget json deserialization" $
        Just (Mainnet01 ^?! versionGenesis . genesisBlockTarget . onChain (unsafeChainId 10)) === (HashTarget . (4 *) <$> decodePowHashNatJson "DOordl9cgfs4ZTBdFnbjRW5th-hW-pL33DIAAAAAAAA")
    , testProperties "mainnet old chains" $
        forChain Mainnet01 maxTarget . unsafeChainId <$> [0..9]
    , testProperties "mainnet new chains" $
        forChain Mainnet01 (Mainnet01 ^?! versionGenesis . genesisBlockTarget . onChain (unsafeChainId 10)) . unsafeChainId <$> [10..19]

    -- testnet20InitialHashTarget properties
    , testProperty "testnet20InitialHashTarget deserialization" $
        Just (Testnet04 ^?! versionGenesis . genesisBlockTarget . onChain (unsafeChainId 10)) === (HashTarget <$> decodePowHashNat64 "NZIklpW6xujSPrX3gyhXInfxxOS6JDjkW_GbGwAAAAA")
    , testProperty "testnet20InitialHashTarget json deserialization" $
        Just (Testnet04 ^?! versionGenesis . genesisBlockTarget . onChain (unsafeChainId 10)) === (HashTarget <$> decodePowHashNatJson "NZIklpW6xujSPrX3gyhXInfxxOS6JDjkW_GbGwAAAAA")
    , testProperties "testnet old chains" $
        forChain Testnet04 maxTarget . unsafeChainId <$> [0..9]
    , testProperties "testnet new chains" $
        forChain Testnet04 (Testnet04 ^?! versionGenesis . genesisBlockTarget . onChain (unsafeChainId 10)) . unsafeChainId <$> [10..19]
    ...
    , testProperty "cross check testnet20InitialHashTarget and mainnet20InitialHashTarget" $
        _hashTarget (Testnet04 ^?! versionGenesis . genesisBlockTarget . onChain (unsafeChainId 10)) `div`
