...
offsetBlockTime offset cut cid =
  add offset $
    maximum $
...
testMineWithPayloadHash ::
  ...
  Cut ->
  IO (Either MineFailure (T2 BlockHeader Cut))
testMineWithPayloadHash db n t ph cid c =
  try $
...
createNewCut hdb n t pay i c = do
  ...
  (h, mc') <-
    extendCut c pay (solveWork work n t)
...
prop_cutBraidingGenesis v =
  either throw (const True) $
