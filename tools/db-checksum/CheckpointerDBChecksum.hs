...
work args = withSQLiteConnection (_sqliteFile args) chainwebPragmas (runReaderT go)
  where
    ...
    getMaxHeight =
      (callDb dbcall $ \db -> qry_ db maxheightstmt [RInt]) >>= \case
    ...
    usertablestmt = \case
      ...
      "[SYS:Modules]" ->
        ...
        \ORDER BY txid DESC, rowkey ASC, rowdata ASC;"
      "[SYS:Namespaces]" ->
        "SELECT * FROM [SYS:Namespaces] \
        \WHERE txid > ? AND txid <= ? \
        \ORDER BY txid DESC, rowkey ASC, rowdata ASC;"
      "[SYS:Pacts]" ->
        "SELECT * FROM [SYS:Pacts] \
        \WHERE txid > ? AND txid <= ? \
        \ORDER BY txid DESC, rowkey ASC, rowdata ASC;"
      tbl ->
        "SELECT * FROM ["
...
getUserTables low high = callDb "getUserTables" $ \db -> do
  ...
  where
    ...
    stmt =
      "SELECT DISTINCT tablename FROM VersionedTableCreation\
      \ WHERE createBlockheight >= ? AND createBlockheight <= ? \
