...
validateConfig o = do
  ...
  when (_configValidate o && isJust (_configChainId o)) $
    throwError $
      "validation (--validate) can only be used if no particular chain is selected"
  where
    chains = chainIds $ _configChainwebVersion o

    checkIfValidChain Nothing = return ()
    checkIfValidChain (Just cid) =
      unless (HS.member cid chains) $
        throwError $
          "Invalid chain id provided: " <> toText cid
...
run config logger = withBlockHeaders logger config $ \pdb x ->
  x
    & progress logg
