  ...
  txStream config mgr logg
    ...
    & S.foldM_
      ( \c _ -> do
          ...
          when (c' `mod` 100 == 0) $
            liftIO $
              logg @T.Text Info ("total tx count: " <> sshow c')
          return c'
      )
      (return 0)
...
prettyCommandWithOutputs p (bh, c, o) =
  T.decodeUtf8 $
    BL.toStrict $
      (if p then encodePretty else encode) $
        object
...
devNetCut config mgr =
  runClientM (cutGetClient ver) (env mgr node) >>= \case
    Left e -> error (show e)
    Right x -> return x
  where
